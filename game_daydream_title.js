...
(function(global){
  ...
  function drawHizanLakeScene(daydream){
    ...
    const x0=55, y0=82, w0=1010, h0=498;
    ...
    ctx.fillText("DAYDREAM FILE  /  0001",92,126);
    ctx.fillStyle="rgba(245,238,224,.94)";
    ctx.font="bold 31px "+FONT_UI;
    ctx.fillText(daydream.title ? daydream.title() : T("白日梦重现","Daydream Reconstruction"),92,166);
    ctx.fillStyle="rgba(245,238,224,.70)";
    ctx.font="bold 24px "+FONT_UI;
    ctx.fillText(daydream.scenarioName ? daydream.scenarioName() : T("Project Zero：日斩","Project Zero: Hizan"),92,202);
    ctx.restore();

    ctx.save();
    ctx.globalAlpha=aPanel;
    roundedRect(ctx,82,250,238,174,12);
    ctx.fillStyle="rgba(24,23,31,.38)";
    ctx.fill();
    ctx.strokeStyle="rgba(245,238,224,.20)";
    ctx.stroke();
    ctx.fillStyle="rgba(245,238,224,.82)";
    ctx.font="bold 14px "+FONT_UI;
    ctx.textAlign="left";
    ctx.fillText(T("调查记录","Investigation Record"),104,280);
    ctx.font="13px "+FONT_UI;
    ctx.fillStyle="rgba(245,238,224,.62)";
    ctx.fillText(T("梦魇图鉴","Nightmare Codex")+"："+(daydream.nightmareCount?daydream.nightmareCount():0)+" / 8",104,318);
    ctx.fillText(T("结局记录","Ending Archive")+"："+(daydream.endingCount?daydream.endingCount():0)+" / 5",104,348);
    ctx.fillText(T("最高线索","Best Clue")+"："+String((daydream.state&&daydream.state.bestClue)||0),104,378);
    ctx.fillStyle="rgba(103,182,184,.78)";
    ctx.font="12px "+FONT_UI;
    ctx.fillText(T("浅湖中心检测到鸟居残响。","Torii residual echo detected at lake center."),104,408);
    ctx.restore();

    drawPZTitleButton(ctx,104,514,128,36,T("梦魇图鉴","Codex"),false,aBtns);
    drawPZTitleButton(ctx,246,514,128,36,T("结局一览","Endings"),false,aBtns);
    drawPZTitleButton(ctx,828,486,184,58,T("开始调查","START"),true,aBtns);
    drawPZTitleButton(ctx,878,104,134,34,T("返回列表","Archive"),false,aBtns*.88);
    ...
    const cards=[
      {
        ...
        tags:[T("梦魇图鉴 0 / 8","NM 0 / 8"),T("结局记录 0 / 5","END 0 / 5"),T("状态：可调查","OPEN")]
      },
      {
    ...
    global.PZDaydream.handleClick = function(){
      ...
      if(this.page === "home"){
        ...
        if(!this.selectedDaydreamScenario){
          // Hizan scenario card
          if(global.clicked && global.inRect && global.inRect(86,222,948,102)){
        ...
        if(global.clicked && global.inRect && global.inRect(878,104,134,34)){
