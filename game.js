...
window.PZ_BUILD_INFO = window.PZ_BUILD_INFO || {
  build: "V49_16_GROWTH_ACTION_RECORD",
  storyModule: true,
  optimized: true
};
...
let renderQuality = "1080P"; // "STANDARD" / "1080P" / "2K" / "AUTO"
...
function detectAutoQuality(){
  ...
  const sw = Math.max(screen.width || 0, window.innerWidth || 0);
  if(cores >= 8 && mem >= 8 && sw >= 2400) return "2K";
  if(cores >= 4 && mem >= 4 && sw >= 1600) return "1080P";
  return "STANDARD";
}
...
const MSG_TEXT = I18N_RES.MSG_TEXT || {
  zh: {
    ...
    eventSupplyClaimed:"废城补给已领取",
    eventSupplySuccess:"领取成功：废城补给",
    codeTooShort:"名字需要2-12个有效字符",
    codeSaved:"名字确认完成",
    guardPrompt:"GUARD! 左键弹刀",
    chainEntry:" 连携入场",
    switchCooldown:"切换冷却 ",
    moveRight:"前往右侧区域",
    missionFailed:"任务失败",
    ...
    tutorialParry:"新手教程 4/4｜弹刀：敌人攻击瞬间点击左键",
  ...
  en: {
    ...
    mailDefault:"Starter mail available",
    eventDefault:"Launch event ongoing",
    warehouseDefault:"Inventory unlocked",
    floraOwned:"Flora already owned",
    floraBought:"Purchase complete: Flora unlocked",
    floraNoCrystal:"Not enough Crystal: Flora costs 3000",
    ...
    eventSupplyClaimed:"Wasteland supply already claimed.",
    eventSupplySuccess:"Claimed: Wasteland Supply",
    codeTooShort:"Name must contain 2-12 valid characters",
    codeSaved:"Name confirmed",
    guardPrompt:"GUARD! Left Click Parry",
    ...
    tutorialParry:"Tutorial 4/4 | Parry: left click at the moment of impact",
...
const LANG_CORE = I18N_RES.LANG_CORE || {
  zh: {
    chapterTitle:"第一章：初入",
    chapterTitleEn:"第一章：初入",
    stagePrefix:"第一章：初入 / 01-",
  ...
  en: {
    ...
    chapterTitleEn:"Chapter 1: Arrival",
    stagePrefix:"Chapter 1: Arrival / 01-",
    ...
    floraPriceLow:"Not enough Crystal: Flora costs 4100",
    ...
    everwinterLow:"Not enough Crystal: Song of Everwinter costs 888",
    ...
    floraFeatureLine:"Circle AoE / Control / Weak single-target",
    ...
    slotPrefix:"Slot ",
    teamHint:"Select up to 3 executors before entering the stage",
    ...
    operatorBuyShop:"Character purchase / Shop",
...
canvas.addEventListener("mouseup", e => { if(e.button===0) { mouseDown = false; mouseAttackConsumed = false; } if(e.button===2) keys["mouse2"]=false; });
...
let language = detectSystemLanguage();
...
const SAVE_KEY = "project_zero_v25_save";
...
function legacyApplyCloudSaveToLocal(saveBox){
  try{
    ...
    const raw = typeof saveBox.raw === "string" ? saveBox.raw : JSON.stringify(saveBox.parsed || saveBox);
...
function getMatchingLocalSaveForAccount(uid){
  try{
    ...
    if(parsed.accountUid !== uid) return null;
    return {raw, parsed};
  }catch(err){
    console.error(err);
    return null;
...
function resetRuntimeForFreshAccount(){
  ...
  if(typeof tutorialCompleted !== "undefined") tutorialCompleted = false;
  if(Array.isArray(owned)) for(let i=0;i<owned.length;i++) owned[i] = (i===0);
  if(Array.isArray(team)){ team[0]=0; team[1]=0; team[2]=0; }
}
async function startLoggedInAccountGame(){
  ...
  try{
    ...
    if(saveBox){
      applyCloudSaveToLocal(saveBox);
      cloudInitialSyncDone = true;
      startLoading(hasCreatedProfile && validPlayerName(playerName) ? "lobby" : "nameInput");
    }else{
...
async function accountLoginFlow(){
  ...
  try{
    ...
    if(saveBox){
      applyCloudSaveToLocal(saveBox);
      cloudInitialSyncDone = true;
      setAccountMsg(accTx("loginSuccess"),100);
      startLoading(hasCreatedProfile && validPlayerName(playerName) ? "lobby" : "nameInput");
    }else{
      const localMatch = getMatchingLocalSaveForAccount(cloudUser.uid);
      if(localMatch){
        ...
        resetRuntimeForFreshAccount();
        cloudInitialSyncDone = true;
        setAccountMsg(accTx("noCloudSave"),150);
        gameMode = "nameInput";
        nameInput = "";
        nameError = "";
      }
...
async function accountRegisterFlow(){
  ...
  try{
    ...
    cloudInitialSyncDone = true;
    resetRuntimeForFreshAccount();
    setAccountMsg(accTx("registerSuccess"),120);
    gameMode = "nameInput";
    nameInput = "";
    nameError = "";
...
function accountGuestFlow(){
  ...
  loadGame();
  if(hasCreatedProfile && validPlayerName(playerName)) startLoading("lobby");
  else{
    gameMode = "nameInput";
    nameInput = "";
    nameError = "";
...
async function bindCurrentLocalSaveToNewAccount(){
  if(accountBusy || !promptAccountCredentials("register")) return;
  if(!await ensureCloudPersistenceReady()) return;
  if(!accountEmail){ setAccountMsg(accTx("enterEmail")); return; }
  if(accountPassword.length < 6){ setAccountMsg(accTx("weakPassword")); return; }
  accountBusy = true;
  setAccountMsg(accTx("syncing"), 120);
  try{
    const cred = await cloudAuth.createUserWithEmailAndPassword(accountEmail, accountPassword);
    cloudUser = cred.user;
    accountAuthed = true;
    guestMode = false;
    cloudInitialSyncDone = true;
    saveGame();
    await autoCloudSaveNow(true);
    setAccountMsg(accTx("registerSuccess"), 140);
  }catch(err){
...
async function autoCloudSaveNow(force=false){
  ...
  if(localSave.raw === cloudLastSyncedRaw){ cloudPendingSave = false; return; }
  cloudBusy = true;
...
async function cancelDeleteRequest(){ setAccountMsg(language==="en" ? "Account deletion is disabled in this build." : "此版本已关闭账号删除。",160); }

async function checkAndCancelPendingDelete(){ setAccountMsg(language==="en" ? "Account deletion is disabled in this build." : "此版本已关闭账号删除。",160); }

async function requestDeleteAccount(){ setAccountMsg(language==="en" ? "Account deletion is disabled in this build." : "此版本已关闭账号删除。",160); }

async function signOutAndClearLocal(){
  cloudSessionToken++;
  if(!await ensureCloudPersistenceReady()) return;
  try{ if(cloudUser && hasCreatedProfile) await autoCloudSaveNow(true); }catch(err){ console.warn("[SignOutSave]", err); }
  try{ await cloudAuth.signOut(); }catch(err){ console.warn("[SignOut]", err); }
  cloudUser = null;
  accountAuthed = false;
  guestMode = false;
  cloudInitialSyncDone = false;
...
async function bootAccountSession(){
  ...
  cloudAuth.onAuthStateChanged(user => {
    ...
    if(cloudUser && gameMode === "login"){
      setAccountMsg(language==="en" ? "Signed in. Click Login to continue." : "已登录，点击登录继续。",90);
    }
  });
}
...
function resetRuntimeDefaults(){
  ...
  protagonistGrowthNotice = "";
  protagonistGrowthNoticeTimer = 0;
  mailClaimed = false;
  eventClaimed = false;
  ...
  actionRecordTaskState = {date:"", done:{login:1}, claimed:{}};
  actionRecordTaskTab = "daily";
...
function saveGame(){
  try{
    ...
    const data = {
      ...
      updatedAt:now,
      accountUid:cloudUser ? cloudUser.uid : "",
      crystals, gold, expBooks, weaponOre, playerLevel, playerExp, playerExpNeed, playerName, playerUID, hasCreatedProfile,
      owned, cleared, charData, lobbyExecutor, renderQuality, targetFPS, monthlyOwned, monthlyClaimed, monthlyClaimDate, mailClaimed, eventClaimed, lastLoginClaimDate, loginClaimIndex,
      loginRewards, levelRewards, boughtPacks, ownedWeapons, weaponInventory, dungeonStamina, dungeonWeeklyCrystalLeft, dungeonCrystalWeekKey, dungeonLastStaminaDate, dungeonCandy, dungeonStimulant, dungeonCandyMonthKey, dungeonCandyDailyUsed, dungeonCandyDailyKey, dungeonRewardMultiplier, materialDungeonDifficulty, materialDungeonSelected, audioMuted, particlesEnabled, damageTextEnabled, tutorialCompleted, language, prologueDone, lobbyGuideDone, achievements, totalKills, totalParries, totalChains, totalBossKills, totalGoldEarned, totalCrystalsEarned, growthGuidePage, growthGuidePageClaimed, growthGuideTaskClaimed, bossMultiplier, bossKrosWeeklyKey, dragonClaw, uiGuideSeen, protagonistStoryLevel, protagonistGrowthNotice, protagonistGrowthNoticeTimer, actionRecordLevel, actionRecordExp, actionRecordExpNeed, actionRecordPage, actionRecordAdvanced, actionRecordUltimate, actionRecordClaimed, actionRecordWeaponChoice, actionRecordTaskState
    };
...
function migrateSaveData(d){
  ...
  if(typeof d.language !== "string" || (d.language !== "zh" && d.language !== "en")) d.language = detectSystemLanguage();
  if(!Array.isArray(d.loginRewards)) d.loginRewards = null;
  if(!Array.isArray(d.levelRewards)) d.levelRewards = null;
}
...
function loadGame(){
  try{
    ...
    else protagonistStoryLevel = clamp(Math.floor(playerLevel||1),1,10);
    if(typeof d.protagonistGrowthNotice === "string") protagonistGrowthNotice = d.protagonistGrowthNotice;
    if(typeof d.protagonistGrowthNoticeTimer === "number") protagonistGrowthNoticeTimer = Math.max(0, d.protagonistGrowthNoticeTimer);
    ...
    if(typeof d.materialDungeonSelected === "number") materialDungeonSelected = clamp(Math.floor(d.materialDungeonSelected),0,2);
    ...
    if(typeof d.actionRecordPage === "number") actionRecordPage = clamp(Math.floor(d.actionRecordPage),0,4);
    ...
    if(d.actionRecordTaskState && typeof d.actionRecordTaskState === "object") actionRecordTaskState = Object.assign({date:"", done:{}, claimed:{}}, d.actionRecordTaskState);
    ...
    }).concat(Array.from({length:10},()=>Array(5).fill(false))).slice(0,10);  }catch(e){}
}
...
const UI_TEXT = I18N_RES.UI_TEXT || {
  zh: {
    ...
    loginEvent:"启程签到", levelEvent:"新手等级", supplyEvent:"废城补给",
    dailyReward:"每日奖励", growthGoal:"成长目标", oneTimeSupply:"一次性补给",
    eventSubtitle:"活动 / 启程计划 / 废城补给",
  ...
  en: {
    click:"CLICK", claimed:"Claimed", claim:"Claim", locked:"Locked", notReached:"Not Reached",
    backLobby:"Back to Lobby", backLogin:"Back to Login", esc:"ESC",
    operation:"Operation", operators:"Executors", shop:"Shop", mail:"Mail", event:"Event", warehouse:"Inventory", settings:"Settings",
    profile:"Profile", graphics:"Graphics", audio:"Audio", account:"Account",
    chapter0:"Chapter 0", main:"Main", combat:"Combat", startAction:"Start", return:"Back",
    ...
    loginEvent:"Journey Check-in", levelEvent:"Rookie Level", supplyEvent:"Wasteland Supply",
    dailyReward:"Daily Reward", growthGoal:"Growth Goal", oneTimeSupply:"One-time Supply",
    eventSubtitle:"Event / Journey Plan / Wasteland Supply",
    settingsSubtitle:"Settings / Graphics / Audio / Account",
    profileSubtitle:"Profile / Click anywhere or Esc to return",
    inventorySubtitle:"Materials and Resources",
    noData:"No Data", achievement:"Achievements", achievementSubtitle:"Journey / Combat / Collection", claimReward:"Claim Reward", completedAchievement:"Completed", achievementLocked:"Locked", shopRecruit:"Recruit", weaponDepot:"Weapon Depot", skin:"Skin", developerSupport:"Support Dev", permanentRecruit:"Permanent", limitedRecruit:"Limited", permanentWeapon:"Permanent Weapons", limitedWeapon:"Limited Weapons", comingSoon:"Coming Soon"
  }
};
...
const MISC_TEXT = I18N_RES.MISC_TEXT || {
  ...
  "en": {
    ...
    "notEnoughCrystal": "Not enough Crystal",
    ...
    "eventLoginSub": "Log in continuously to claim combat supplies",
    ...
    "teamLeader": "Leader / Initial Executor",
    "teamSwitchable": "Switch-in Executor",
    "notOwned": "Not Owned",
    "clickToJoin": "Click to Join",
    "levelUp": "Level Up",
    "upgradeWeapon": "Upgrade Weapon",
    "notCleared": "Not cleared yet",
    ...
    "eventPanelSupplyTitle": "Supply Operation",
...
Object.assign(MISC_TEXT.en, {
  ...
  "supportDesc": "Thanks for supporting Ravenhado. Support items will not provide combat advantages.",
  "bought": "Bought",
  "operatorListHint": "Select an executor to enter growth page",
...
function localizeText(text){
  ...
  const exact = {
    ...
    "GUARD! 左键弹刀":"GUARD! Left Click Parry",
    ...
    exact["角色购买 / 商店"] = "Character purchase / Shop";
    exact["升级角色 / 强化武器"] = "Level characters / Upgrade weapons";
    exact["角色升级材料"] = "Character level material";
    exact["武器强化材料"] = "Weapon upgrade material";
    exact["开服补给"] = "Launch Supply";
...
function protagonistSyncStatusText(){
  ...
  if(cap >= 60 && protagonistStoryLevel >= cap) return language==="en" ? "Main story sync completed" : "主线同步已完成";
  if(protagonistStoryLevel >= cap) return language==="en" ? "Reached current chapter cap" : "已达到当前章节等级上限";
  return language==="en" ? "Growth follows main stages" : "随主线关卡自动成长";
}
function protagonistSkillLevelFromLevel(lv){
  // Main character skills also follow stage growth, not manual materials.
  if(lv >= 50) return 10;
  if(lv >= 40) return 8;
  if(lv >= 30) return 6;
  if(lv >= 20) return 4;
...
function setProtagonistStoryLevel(lv, silent=false){
  const cap = protagonistStoryLevelCap();
  const next = clamp(Math.floor(lv||1),1,cap);
  if(next > protagonistStoryLevel){
    protagonistStoryLevel = next;
    playerLevel = protagonistStoryLevel; // legacy compatibility only
    protagonistGrowthNotice = (language==="en"?"Protagonist synced to Lv.":"主角同步至 Lv.") + protagonistStoryLevel;
    protagonistGrowthNoticeTimer = silent ? 0 : 130;
    if(!silent && typeof showCenter === "function") showCenter(protagonistGrowthNotice, 100);
    saveGame();
  }else{
    protagonistStoryLevel = Math.max(1, protagonistStoryLevel);
    playerLevel = protagonistStoryLevel;
  }
}
function syncProtagonistLevelByStage(stage){
  const ch = currentStoryChapterForGrowth();
  if(ch <= 0){
    // Chapter 0: total growth is Lv.1 -> Lv.10.
    setProtagonistStoryLevel(Math.min(10, Math.max(1, Math.floor(stage||1)+1)));
  }else{
    const base = ch*10;
    setProtagonistStoryLevel(Math.min(protagonistStoryLevelCap(), base + Math.max(1, Math.floor(stage||1))));
  }
}
...
function buildStageText(id){
  const source = language==="en" ? window.PZ_CHAPTER0_STAGES_EN : window.PZ_CHAPTER0_STAGES_ZH;
...
const roles = [
  {name:"凯恩", color:"#ff5757", sub:"#ffbe5c", atk:[14,18,36], skill:65, speed:3.1, style:"物理剑卫", line:"烈阳，斩开黑夜。"},
  {name:"艾洛", color:"#74ffb7", sub:"#7cc7ff", atk:[10,14,28], skill:48, speed:3.4, style:"风系辅助", line:"风会记住这一击。"},
  {name:"诺克斯", color:"#b47cff", sub:"#ffffff", atk:[18,24,46], skill:84, speed:2.9, style:"暗系击破", line:"无名之刃，撕裂终局。"},
  {name:"芙洛拉", color:"#88d8ff", sub:"#ffffff", atk:[16,20,42], skill:78, speed:3.0, style:"冰系法术", line:"霜影落下，万物静止。"},
  {name:"主角", color:"#dfe6ef", sub:"#313846", atk:[15,20,38], skill:72, speed:3.05, style:"单体输出", line:"灰白之间，斩开前路。"}
];
...
const BREAK_LEVEL_CAPS=[20,40,60];
const BREAK_COSTS=[{mat:10,gold:3000},{mat:20,gold:8000}];
function roleBreakStage(i){ return (charData[i]&&charData[i].breakStage)||0; }
function roleLevelCap(i){ return isProtagonist(i)?protagonistLevel():BREAK_LEVEL_CAPS[Math.min(roleBreakStage(i),2)]; }
function canBreakthrough(i){ return !isProtagonist(i)&&roleDisplayLevel(i)>=roleLevelCap(i)&&roleBreakStage(i)<2; }
...
let playerLevel = 1; // Legacy compatibility only. No longer shown as player level.
let playerExp = 0;
let playerExpNeed = 1000;
let protagonistStoryLevel = 1;
let protagonistGrowthNotice = "";
let protagonistGrowthNoticeTimer = 0;
...
let actionRecordTaskState = {date:"", done:{}, claimed:{}};
let actionRecordTaskTab = "daily";
...
const stages = [
  ...
  ["意识坠落","Consciousness Collapse",75,false,"逃离途中，两人瞬间瘫倒，意识被拖入未知空间。","During the escape, both of them suddenly collapse as their minds are pulled somewhere unknown."],
  ["末日景象","Endscape",75,false,"隐者醒来，看见末日般的世界，并发现凯恩不见了。","Hermit wakes in a ruined world and realizes Kane is gone."],
  ["晶体人","Crystal Man",75,false,"隐者面前出现晶体人，必须应战。","A crystalized human figure appears before Hermit, forcing a fight."],
  ["不同的世界","Different Worlds",100,false,"隐者与凯恩再次相遇，但两人看见的是截然不同的世界。","Hermit finds Kane again, but each of them is seeing a completely different world."],
  ...
  ["白日梦中层","Middle Layer",100,false,"两人进入白日梦中层，巨大的结晶开始孕育怪物，凯恩头痛欲裂。","They reach the middle layer of the Daydream, where massive crystals begin giving birth to monsters."],
...
].map((s,i)=>({id:i+1,zh:s[0],name:s[1],reward:s[2],boss:s[3],zhDesc:s[4],desc:s[5]}));
const missionTypes = ["annihilation","survival","elite","mixed","elite","mixed","annihilation","survival","mixed","chapterBoss","annihilation"];
...
function stageCode(id){ return tx("stageCodePrefix") + String(id).padStart(2,"0"); }
...
function rebuildStoryScripts(){
  ...
  const source = language==="en" ? window.PZ_CHAPTER0_STORY_EN : window.PZ_CHAPTER0_STORY_ZH;
  if(source){
...
function stabilizePlayerStats(){
  ...
  playerExpNeed = Math.max(100, Math.floor(safeNumber(playerExpNeed,1000)));
  ...
  if(typeof gameMode !== "undefined" && gameMode === "battle") saveCurrentRoleHp();
...
function createEnemy(x,y,boss=false,type="normal"){
  type = type || "normal";
  const commissionStage = selectedTab==="combat" ? commissionStages[selectedStage-1] : null;
...
function spawnArea(){
  ...
  if(t==="chapterBoss"){
    ...
    else { enemies.push(createEnemy(700,H/2+90,true,"boss")); if(selectedStage>=8) enemies.push(createEnemy(500,H/2+150,false,"shield")); }
  } else if(t==="survival"){
    ...
    if(area===3) enemies.push(createEnemy(860,H/2+60,false,"elite"));
  } else if(t==="ranged"){
    ...
    enemies.push(createEnemy(810,H/2+30,false,area===3?"ranged":"normal"));
    if(t==="bossPrep" && area===3) enemies.push(createEnemy(720,H/2+160,false,"shield"));
  } else {
...
function arEnsureTaskState(){
  const k = arTodayKey();
  if(!actionRecordTaskState || typeof actionRecordTaskState !== "object") actionRecordTaskState = {date:k,done:{},claimed:{}};
  if(actionRecordTaskState.date !== k) actionRecordTaskState = {date:k,done:{},claimed:{}};
  actionRecordTaskState.done = actionRecordTaskState.done || {};
  actionRecordTaskState.claimed = actionRecordTaskState.claimed || {};
  actionRecordTaskState.done.login = Math.max(1, actionRecordTaskState.done.login||0);
}
function addActionRecordTaskProgress(id, amount=1){
  arEnsureTaskState();
  actionRecordTaskState.done[id] = (actionRecordTaskState.done[id] || 0) + amount;
  saveGame();
}
function actionRecordExpNeedFor(lv){ return 500; }
function addActionRecordExp(amount, reason=""){
  amount = Math.max(0, Math.floor(amount||0));
  if(amount<=0 || actionRecordLevel>=ACTION_RECORD_MAX_LEVEL) return;
  ...
  while(actionRecordLevel < ACTION_RECORD_MAX_LEVEL && actionRecordExp >= actionRecordExpNeed){
    ...
    actionRecordLevel++;
    actionRecordExpNeed = actionRecordExpNeedFor(actionRecordLevel);
    actionRecordPage = clamp(Math.floor((actionRecordLevel-1)/ACTION_RECORD_LEVELS_PER_PAGE),0,4);
    showCenter((language==="en"?"Action Record Lv.":"行动记录 Lv.")+actionRecordLevel, 80);
  }
  saveGame();
}
function addPlayerExp(amount){
  // V49.16: old Player EXP is redirected to Action Record EXP.
  addActionRecordExp(amount, "legacy");
  if(amount>0) addActionRecordTaskProgress("battle",1);
}
...
function claimAchievement(id){
  ...
  st.claimed = true;
  crystals += a.crystals;
  totalCrystalsEarned += a.crystals;
  achievementMsg = mt("claimedPrefix") + achievementName(a) + " +" + a.crystals;
  sfx("reward");
  saveGame(); autoCloudSaveNow(true);
}
...
function grantReward(pack){
  if(pack.crystals){ crystals += pack.crystals; totalCrystalsEarned += pack.crystals; }
...
function rewardText(pack){
  const arr = [];
  if(pack.crystals) arr.push((language==="en"?"Crystal+":"水晶+")+pack.crystals);
...
function ensureBattleRoleHp(){
  ...
  for(let i=0;i<roles.length;i++){
    ...
    if(!Number.isFinite(battleRoleHp[i]) || battleRoleHp[i] <= 0){
...
function damageCurrentRoleHp(amount, label="HIT", color="#ff5555"){
  ...
  if(player.hp<=0){ failMission(); return true; }
  return false;
}

function setBattleRole(newRole){
  // V43.3: Role HP is independent. No ratio sharing, no syncing values.
  saveCurrentRoleHp();
...
function startBattle(){
  ...
  syncPlayerHpFromRole();
  player.energy=80; resetUltForBattle(); player.x=150; player.y=H/2+115; player.vx=0; player.vy=0;
  player.attackCd=0; player.skillCd=0; player.ultCd=0; player.dashCd=0; player.switchCd=0; chainSelect=false; chainSelectTimer=0; player.inv=0; player.chain=0; player.chainTimer=0; player.guardTimer=0; player.parryReady=0; player.parryTarget=null; player.perfectBuff=0; player.perfectDodgeTimer=0; combo=0; comboTimer=0; stylishScore=0; combatRank="D"; actionPromptTimer=0; chainReady=false; chainTarget=null; area=1; areaCleared=false; commissionComplete=false; areaDialogueShown={}; particles=[]; slashes=[]; texts=[];
  spawnArea(); showCenter(battleModeSource==="projectArea" ? "Project Area" : (battleModeSource==="materialDungeon" ? ((language==="en"?"Material Dungeon":"材料副本")+" "+roman(materialDungeonDifficulty)) : (battleModeSource==="commission" ? ("C-"+String(selectedStage).padStart(2,"0")) : stageCode(selectedStage))),70);
}
function enterLobby(){
  releaseMobileButtons();
  clearMobileMoveKeys(); clearTransientBattleState(); gameMode="lobby"; notice=prologueDone && !lobbyGuideDone ? msg("lobbyGuide") : msg("defaultNotice"); }
function enterOperation(){ gameMode="operation"; selectedTab="main"; }
function enterStory(id){ rebuildStoryScripts(); selectedStage=id; currentStory=storyScripts[id] || [[mt("storyFallbackSpeaker"),mt("storyFallbackText")]]; storyIndex=0; gameMode="story"; }
...
function floraAttack(){
  if(player.parryReady>0){ parryCounter(); return; }
...
function hitEnemy(e,dmg,knock=8,stunDmg=16,color="#fff",label=null){
  ...
  combo++; comboTimer=100; addStyle(label==="PARRY!"?65:label==="ULT"?80:label==="3rd HIT"?35:18); player.energy=clamp(player.energy+8,0,100); gainUlt(120, "hit");
...
function panelDamage(roleId, multiplier, kind="normal", randomBonus=0){
  ...
  return Math.max(1, Math.floor(atk * multiplier * growth + randomBonus));
}
...
function chainAttack(){
  ...
  // switch to next team member for flashy assist
  let idx = team.indexOf(player.role);
  if(idx < 0) idx = 0;
  setBattleRole(team[(idx+1)%team.length]);
...
function protagonistSkill(){
  ...
  const lv=protagonistLevel(), skl=protagonistSkillLevel();
  sfx("skill");
  showCenter(language==="en"?"CORE OVERLOAD":"核心过载",28);
  doShake(10);
  doHitStop(4);
  if(target && withinDist(player.x,player.y,target.x,target.y,260)){
    player.facing=target.x>=player.x?1:-1;
    addSlash(target.x,target.y,130,"#ffffff",20,"skill");
    addSlash(target.x,target.y,80,"#8c8f96",16,"skill");
    addParticles(target.x,target.y,"#ffffff",12,5);
    hitEnemy(target,panelDamage(player.role,2.15,"skill",Math.random()*45),20,panelShieldDamage(player.role,72,"skill"),"#ffffff","CORE");
  }
}
...
function protagonistUltimateResolve(){
  const target=lockTarget && lockTarget.alive ? lockTarget : nearestEnemy();
  const lv=protagonistLevel(), skl=protagonistSkillLevel();
  if(!target){ addSlash(player.x,player.y,220,"#ffffff",28,"ultimate"); return; }
  player.x=clamp(target.x-72,45,W-45);
  player.y=clamp(target.y,110,H-45);
  player.facing=1;
  addSlash(target.x,target.y,240,"#ffffff",30,"ultimate");
  addSlash(target.x,target.y,155,"#8c8f96",24,"ultimate");
  addParticles(target.x,target.y,"#ffffff",24,6);
  doShake(24);
  doHitStop(5);
  flash=14;
  hitEnemy(target,panelDamage(player.role,5.20,"ultimate",Math.random()*120),32,panelShieldDamage(player.role,165,"ultimate"),"#ffffff","GRAY DOMAIN");
  addText(target.x,target.y-120,language==="en"?"GRAY DOMAIN":"灰域斩击","#ffffff",true);
}
...
function attack(){
  ...
  for(const e of enemies){
    if(e.alive && withinDist(sx,player.y,e.x,e.y,range)){
      ...
      hitEnemy(e,panelDamage(player.role,[0.72,0.90,1.35][step-1],"normal",Math.random()*18),step===3?20:8,step===3?panelShieldDamage(player.role,46,"normal"):panelShieldDamage(player.role,18,"normal"),color,step===3?"3rd HIT":null);
    }
  }
}
function skill(){
  ...
  sfx("skill"); showCenter("E SKILL",24); doShake(11); doHitStop(4);
  for(const e of enemies) if(e.alive && withinDist(sx,player.y,e.x,e.y,135)) hitEnemy(e,panelDamage(player.role,2.05,"skill",Math.random()*35),16,panelShieldDamage(player.role,45,"skill"),role.color);
}
function ultimate(){
  if(player.ultCd>0 || player.ult<ULT_MAX || ult.active){ if(player.ult<ULT_MAX) showCenter(mt("ultimateNotReady"),28); return; }
  player.ult=0; player.ultCd=210; player.inv=100; ult={active:true,timer:0,role:player.role,hitDone:false}; sfx("ultStart"); showCenter("ULTIMATE",35);
}
function resolveUltimate(){
  ...
  const role=roles[ult.role]; const cd=charData[ult.role]; addSlash(player.x,player.y,260,role.color,28,"ultimate"); addSlash(player.x,player.y,180,"#fff",22,"ultimate");
...
function switchRole(){
  ...
  const pe=findParryEnemy();

  if(pe){
    player.switchCd = Math.max(SWITCH_CD_FRAMES, 72);
    pe.windup=0; pe.attackCd=Math.max(pe.attackCd, pe.boss?120:95); pe.parried=28;
    player.guardTimer=28; player.parryReady=42; player.parryTarget=pe; player.inv=32;
    player.facing=pe.x>=player.x?1:-1;
    sfx(pe.boss ? "parryBoss" : "parry"); showCenter(msg("guardPrompt"),45);
    addText(player.x,player.y-42,pe.boss?"PERFECT GUARD":"TING! GUARD","#ffe082",true);
    spawnParrySpark(player.x+player.facing*42, player.y-4, pe.boss?28:18, !!pe.boss);
    addSlash(player.x+player.facing*38,player.y,pe.boss?128:96,"#ffe082",22,"parry");
    doShake(pe.boss?16:10); doHitStop(pe.boss?9:6);
    slowMo=12;
    return;
  }
...
function fixRuntimeValues(){
  ...
  if(!Number.isFinite(playerExpNeed) || playerExpNeed < 1) playerExpNeed = 1000;
...
function validPlayerName(s){
  return /^[\u4e00-\u9fa5a-zA-Z0-9]{2,12}$/.test(String(s || ""));
}
...
function updateNameInput(){
  ...
  if(clicked){
    if(inRect(W/2-190, 283, 380, 58)){
      const v = window.prompt(tx("nameInputTitle"), nameInput || "");
      if(v !== null){
        nameInput = cleanPlayerName(v);
        nameError = validPlayerName(nameInput) ? "" : msg("nameNeedOne");
      }
    }
...
function drawLoading(){
  ...
  ctx.font="13px " + FONT_UI;
  ctx.fillText("System Check / Save Check / Lobby Initialization",W/2,H-60);
}
...
function tutorialTextPack(){
  return language === "en" ? {
    ...
    area3Text:"Fight the training target. Try left click attacks, E skill, Shift dash, and parry during the enemy warning.",
    completeTitle:"Tutorial Complete",
    completeText:"You have completed basic movement, attacks, exploration, skill, dash, and parry training.",
    moveObj:"Objective: move to the blue marker.",
    crateObj:"Objective: left click near the crates to break them.",
    combatObj:"Objective: defeat the training target. E = skill, Shift = dash.",
    parryHint:"Enemy warning! Left click near the target to parry.",
    ...
    area3Text:"击败训练目标。尝试左键攻击、E 技能、Shift 冲刺，并在敌人预警时弹刀。",
    completeTitle:"教程完成",
    completeText:"你已经完成移动、攻击、探索、技能、冲刺和弹刀基础教学。",
    moveObj:"目标：移动到蓝色光圈。",
    crateObj:"目标：靠近木箱并左键攻击打碎它们。",
    combatObj:"目标：击败训练目标。E = 技能，Shift = 冲刺。",
    parryHint:"敌人预警！靠近目标左键可弹刀。",
...
function drawTutorialObjective(text){
  ctx.save();
  ctx.fillStyle="rgba(0,0,0,.62)";
  ctx.fillRect(30,28,640,60);
  ctx.strokeStyle="rgba(255,255,255,.14)";
  ctx.strokeRect(30,28,640,60);
  ctx.fillStyle="#ffe066";
...
function startTutorialBattle(){
  ...
  player.x=165; player.y=H/2+120; player.hp=100; player.energy=100; player.ult=0; player.inv=0;
...
function spawnTutorialEnemy(){
  ...
  tutorialEnemy.hp = 220;
  tutorialEnemy.maxHp = 220;
...
function updateTutorialBattle(){
  ...
  const speed = roles[player.role].speed * MOVE_SPEED_MULT * 0.88;
  let mx=0,my=0;
  ...
  if(tutorialStep>=4 && tutorialStep<8){
    ...
    if(e && e.alive){
      ...
      if(mouseDown && !mouseAttackConsumed){
        mouseAttackConsumed=true;
        if(e.warning && withinDist(player.x,player.y,e.x,e.y,105)){
          tutorialParried = true;
          e.hp -= 36;
          e.attackCd = 112;
          e.warning=false;
          sfx("parry");
          showCenter("PERFECT GUARD",55);
          addText(e.x,e.y-45,"PARRY","#7cc7ff",true);
          addSlash(e.x,e.y,130,"#7cc7ff",18,"guard");
          addParticles(e.x,e.y,"#7cc7ff",14,5);
          doShake(8);
        }else if(withinDist(player.x,player.y,e.x,e.y,112)){
          e.hp -= 18;
          sfx("slash2");
...
function drawTutorialBattle(){
  ...
  if(tutorialStep>=5 && tutorialStep<8){
    ctx.save();
    ctx.fillStyle="rgba(0,0,0,.50)";
    ctx.fillRect(30,94,320,94);
    ctx.strokeStyle="rgba(255,255,255,.12)";
    ctx.strokeRect(30,94,320,94);
    ctx.font="15px " + FONT_UI;
    ctx.textAlign="left";
    ctx.fillStyle=tutorialUsedSkill?"#7cc7ff":"#aaa";
    ctx.fillText((tutorialUsedSkill?"✓ ":"□ ") + (language==="en"?"Use E skill":"使用 E 技能"),50,124);
    ctx.fillStyle=tutorialUsedDash?"#7cc7ff":"#aaa";
    ctx.fillText((tutorialUsedDash?"✓ ":"□ ") + (language==="en"?"Use Shift dash":"使用 Shift 冲刺"),50,150);
    ctx.fillStyle=tutorialParried?"#7cc7ff":"#aaa";
    ctx.fillText((tutorialParried?"✓ ":"□ ") + (language==="en"?"Parry once":"完成一次弹刀"),50,176);
    ctx.restore();
  }
...
function updateLogin(){
  ...
  if(clicked){
    ...
    if(inRect(panelX+45,panelY+103,330,44)){
      const v=window.prompt(accTx("enterEmail"), accountEmail || "");
      if(v!==null) accountEmail=String(v).trim();
    }
    else if(inRect(panelX+45,panelY+175,330,44)){
      const v=window.prompt(accTx("enterPassword"), accountPassword || "");
      if(v!==null) accountPassword=String(v);
    }
...
const UI_GUIDE_TEXT = {
  ...
  operators:{
    ...
    body:{zh:"执行官页面用于查看角色、升级等级、强化技能与武器。主角由主线关卡自动成长，不消耗养成资源。",en:"The Operators page lets you view characters, level them up, and improve skills and weapons. The protagonist grows automatically through main stages and uses no growth materials."}
  },
  shop:{
  ...
  settings:{
    ...
    body:{zh:"设置用于调整语言、画质、帧率、音效与账号相关选项。教程回看未来会放入档案功能，不放在设置里。",en:"Settings controls language, quality, FPS, audio, and account options. Tutorial replay will belong to the future Archive, not Settings."}
  },
  mainStage:{
  ...
  commission:{
    ...
    body:{zh:"委托作战是额外战斗任务，适合获取水晶、经验与培养资源。",en:"Commission Operation provides extra combat missions for Crystal, EXP, and growth resources."}
  },
  dungeon:{
...
function requestFeatureGuide(key, enterFn){
  ...
  uiGuidePrompt = {key:key, enter:enterFn};
  sfx("ui");
  return true;
}
...
function updateFeatureGuidePrompt(){
  ...
  if(clicked){
    const x=W/2-315, y=H/2-170;
    if(inRect(x+58,y+255,170,46)){
      showCenter(language==="en" ? "Tutorial will be added to Archive later" : "完整教程将在未来加入档案", 90);
      closeFeatureGuidePrompt(true, true);
      return true;
    }
    if(inRect(x+250,y+255,170,46)){
      closeFeatureGuidePrompt(true, true);
      return true;
    }
    if(inRect(x+442,y+255,130,46) || inRect(x+590,y+18,30,30)){
      closeFeatureGuidePrompt(false, false);
      return true;
    }
  }
  if(justPressed("escape")){
...
function drawFeatureGuidePrompt(){
  ...
  const x=W/2-315, y=H/2-170, w=630, h=340;
  ctx.save();
  ctx.fillStyle="rgba(0,0,0,.58)";
  ctx.fillRect(0,0,W,H);
  ctx.fillStyle="rgba(7,10,22,.96)";
  ctx.fillRect(x,y,w,h);
  ctx.strokeStyle="rgba(255,224,102,.70)";
  ctx.lineWidth=2;
  ctx.strokeRect(x,y,w,h);
  ...
  ctx.font="bold 24px " + FONT_UI;
  ctx.textAlign="left";
  ctx.fillText(title,x+38,y+58);

  ctx.fillStyle="rgba(255,255,255,.78)";
  ctx.font="16px " + FONT_UI;
  wrapText(body,x+38,y+100,w-76,28);

  ctx.fillStyle="rgba(124,199,255,.76)";
  ctx.font="13px " + FONT_UI;
  ctx.fillText(language==="en" ? "This explanation appears only when you actively enter a feature for the first time." : "该说明只会在你首次主动进入该功能时出现。",x+38,y+205);
  ctx.fillText(language==="en" ? "Full tutorial replay will be placed in Archive in a future version." : "完整教程回看未来会放入档案功能。",x+38,y+228);

  drawBtn(language==="en"?"Continue":"我知道了","",x+210,y+255,210,46,true,"#ffe066");
  ctx.fillStyle="rgba(255,255,255,.78)";
  ctx.font="bold 20px Arial";
  ctx.textAlign="center";
  ctx.fillText("×",x+605,y+42);
  ctx.restore();
}
...
function updateLobby(){
  ...
  const px=W-255;
  if(!lobbyGuideDone && prologueDone) notice = msg("lobbyGuide");
  if(clicked){
    if(prologueDone && !lobbyGuideDone){ lobbyGuideDone = true; saveGame(); }
    if(inRect(760,30,145,46)){ openStaminaRecover(); clicked=false; return; }
    if(dist(mouseX, mouseY, 312, 310) < 22){ nextLobbyRole(); clicked=false; return; }
    if(dist(mouseX, mouseY, W-162, 24) < 34){ gameMode="actionRecord"; clicked=false; return; }
    if(dist(mouseX, mouseY, W-112, 24) < 34){ gameMode="growthGuide"; clicked=false; return; }
    if(dist(mouseX, mouseY, W-62, 24) < 34){ gameMode="achievements"; clicked=false; return; }
    if(inRect(28,30,360,116)) gameMode="profile";
    else if(inRect(px+22,118,208,56)) requestFeatureGuide("operation", function(){ enterOperation(); });
    else if(inRect(px+22,190,208,56)) requestFeatureGuide("operators", function(){ gameMode="operators"; });
    else if(inRect(px+22,262,208,56)) requestFeatureGuide("shop", function(){ shopMsg = msg("shopDefault"); gameMode="shop"; });
    else if(inRect(px+22,334,208,56)) requestFeatureGuide("mail", function(){ gameMode="mail"; });
    else if(inRect(px+22,406,208,56)) requestFeatureGuide("event", function(){ eventMsg = msg("eventDefault"); gameMode="event"; });
    else if(inRect(px+22,478,208,56)) requestFeatureGuide("warehouse", function(){ warehouseMsg = msg("warehouseDefault"); gameMode="warehouse"; });
    else if(inRect(px+22,550,208,56)) requestFeatureGuide("settings", function(){ gameMode="settings"; });
  }
  clicked=false;
}
...
function claimMonthlyCardReward(){
  ...
  monthlyClaimed = true;
  shopMsg = msg("monthlyClaimSuccess");
  sfx("reward");
  saveGame();
...
function normalizeDungeonRuntime(){
  ...
  materialDungeonSelected = clamp(Math.floor(materialDungeonSelected || 0), 0, 2);
}

function dungeonModes(){
  return language === "en" ? [
    {name:"Material", full:"Material Dungeon", icon:"◆", color:"#7cc7ff", desc:"Gold, character EXP materials, and weapon materials."},
    {name:"Boss", full:"Boss Challenge", icon:"B", color:"#ff6b9b", desc:"Challenge powerful enemies for rare rewards. Coming soon."},
    {name:"Explore", full:"Exploration Map", icon:"◇", color:"#7cffb2", desc:"Explore areas, chests, crates, and one-time rewards. Coming soon."},
...
function materialDungeons(){
  return language === "en" ? [
    ...
    {name:"Weapon Material", short:"Weapon", type:"weapon", color:"#7cc7ff", desc:"Obtain weapon upgrade materials.", monster:"Crystal Guard", base:{gold:650, expBooks:0, weaponOre:4}}
  ] : [
...
function drawDungeonTopBar(){
  ...
  ctx.font="bold 16px " + FONT_UI;
  ctx.textAlign="left";
  ctx.fillText((language==="en"?"Stamina ":"体力 ")+Math.floor(dungeonStamina)+"/240",W-372,61);
  ctx.fillStyle="#ffe066";
...
function drawMaterialDungeonContent(){
  ...
  if(r.expBooks) { ctx.fillText((language==="en"?"Character Material ×":"角色升级材料 ×")+r.expBooks,x,ry); ry+=24; }
  if(r.weaponOre) { ctx.fillText((language==="en"?"Weapon Material ×":"武器强化材料 ×")+r.weaponOre,x,ry); ry+=24; }
...
function updateOperation(){
  ...
  if(clicked){
    ...
    if(inRect(90,595,135,38)){
      handled = setOperationTabWithGuide("main", "mainStage");
    }else if(inRect(240,595,135,38)){
      handled = setOperationTabWithGuide("combat", "commission");
    }else if(inRect(390,595,135,38)){
      handled = setOperationTabWithGuide("dungeon", "dungeon", function(){ dungeonSelected=0; });
    }else if(inRect(540,595,135,38)){
      handled = setOperationTabWithGuide("sideStory", "sideStory");
    }else if(inRect(690,595,135,38)){
      handled = setOperationTabWithGuide("daydream", "daydream", function(){ if(window.PZDaydream && typeof window.PZDaydream.startEntryAnimation === "function") window.PZDaydream.startEntryAnimation(); });
    ...
    if(!handled && operationDetailVisible && selectedTab!=="dungeon" && selectedTab!=="sideStory" && selectedTab!=="daydream"){
      const locked = selectedTab==="combat" ? false : (selectedStage>1 && !cleared[selectedStage-1]);
    ...
    if(!handled && selectedTab!=="dungeon" && selectedTab!=="sideStory" && selectedTab!=="daydream"){
      ...
      for(let i=0;i<list.length;i++){
        ...
        const locked = selectedTab==="combat" ? false : (i>0&&!cleared[i]);
...
function updateStory(){
  menuPulse++;
  if(clicked && inRect(W-205,H-82,165,48)){ clicked=false; mouseDown=false; enterTeamSetup(); return; }
  if(clicked || justPressed("enter") || justPressed(" ")){ storyIndex++; if(storyIndex>=currentStory.length) enterTeamSetup(); }
  if(justPressed("escape")) enterTeamSetup();
  clicked=false;
}
...
function updateOperators(){
  ...
  if(clicked){
    if(operatorPageMode==="list"){
      ...
      for(let idx=0; idx<order.length; idx++){
        ...
        if(inRect(x,y,cardW,cardH)){ selectedOperator=i; if(owned[i]) player.role=i; operatorPageMode="detail"; clicked=false; return; }
      }
    }else{
      if(inRect(46,36,54,42) || inRect(60,592,190,46)){ operatorPageMode="list"; clicked=false; return; }
      const rx=780, ry=112, rh=506;
      if(inRect(rx+18,ry+20,68,36)) operatorTab="level";
      else if(inRect(rx+94,ry+20,68,36)) operatorTab="skill";
      else if(inRect(rx+170,ry+20,68,36)) operatorTab="weapon";
      else if(inRect(rx+246,ry+20,68,36)) operatorTab="break";
      ...
      if(!(!owned[i]) && !isProtagonist(i) && inRect(bx,by,222,46)){
        if(operatorTab==="level"){
          const costGold=200+cd.level*40;
          if(canBreakthrough(i)){ performBreakthrough(i); } else if(expBooks>0 && gold>=costGold && cd.level<roleLevelCap(i)){ expBooks--; gold-=costGold; cd.level++; cd.skillPoints++; saveGame(); }
  ...
  if(justPressed("escape")){ if(operatorPageMode==="detail") operatorPageMode="list"; else enterLobby(); }
  clicked=false;
}
function updateShop(){
  menuPulse++;
  ...
  if(clicked){
    ...
    if(shopTab==="recommend"){
      if(inRect(70,235,660,220)){ shopTab = "recommend"; shopSubTab="limited"; shopMsg=tr("已前往限定招募","Switched to limited recruit"); }
      if(inRect(760,235,250,100)){ shopTab="monthly"; shopMsg=tr("已前往月卡","Switched to monthly pass"); }
      if(inRect(760,355,250,100)){ shopTab="packs"; shopMsg=tr("已前往礼包","Switched to packs"); }
    }
    ...
    if(shopTab==="weapon"){
      if(shopSubTab==="limited" && inRect(70,250,420,210)){
    ...
    if(shopTab==="crystal"){
      const crystalPacks=[
        {x:70, amount:500, msg:"模拟购买成功：+500水晶"},
        {x:255, amount:1500, msg:"模拟购买成功：+1500水晶"},
        {x:440, amount:3500, msg:"模拟购买成功：+3500水晶"},
        {x:625, amount:5500, msg:"模拟购买成功：+5500水晶"},
        {x:810, amount:7500, msg:"模拟购买成功：+7500水晶"}
      ];
      for(const p of crystalPacks){
        if(inRect(p.x,245,165,130)){
          crystals+=p.amount;
          totalCrystalsEarned+=p.amount;
          shopMsg=p.msg;
          sfx("buy");
          saveGame(); autoCloudSaveNow(true);
        }
...
function updateBattle(){
  ...
  gainUlt(0.35*frameScale, "passive");
  ...
  if(justPressed("r"))switchRole(); if(justPressed("tab"))toggleLock(); if(justPressed("f")){ if(!projectAreaInteract()) chainAttack(); } if(justPressed("escape")){ clearTransientBattleState(); gameMode="operation"; }
  ...
  player.attackCd=Math.max(0,player.attackCd-frameScale); attackInputLock=Math.max(0,attackInputLock-frameScale); attackBuffer=Math.max(0,attackBuffer-frameScale); skillBuffer=Math.max(0,skillBuffer-frameScale); ultBuffer=Math.max(0,ultBuffer-frameScale); dashBuffer=Math.max(0,dashBuffer-frameScale); player.skillCd=Math.max(0,player.skillCd-frameScale); player.ultCd=Math.max(0,player.ultCd-frameScale); player.dashCd=Math.max(0,player.dashCd-frameScale); player.switchCd=Math.max(0,player.switchCd-frameScale); player.inv=Math.max(0,player.inv-frameScale); player.chainTimer=Math.max(0,player.chainTimer-frameScale); player.guardTimer=Math.max(0,player.guardTimer-frameScale); player.parryReady=Math.max(0,player.parryReady-frameScale); player.perfectBuff=Math.max(0,player.perfectBuff-frameScale); if(player.parryReady<=0)player.parryTarget=null;
  for(const e of enemies){
    if(!e.alive)continue; e.parried=Math.max(0,e.parried-frameScale); e.breakLock=Math.max(0,(e.breakLock||0)-frameScale); e.freeze=Math.max(0,(e.freeze||0)-frameScale); e.chill=Math.max(0,(e.chill||0)-frameScale);
    ...
    if(e.stun<=0){ e.vx*=.84; e.vy*=.84; e.stun++; if(e.stun>=0)e.stun=e.maxStun; }
      ...
      if(e.boss && e.hp < e.maxHp*0.55 && e.phase===1){ e.phase=2; e.shield=Math.max(e.shield,900); e.maxShield=Math.max(e.maxShield,900); showActionPrompt(msg("bossPhase2"),90); }
      if(e.boss && e.hp < e.maxHp*0.25 && e.phase===2){ e.phase=3; e.rage=true; showActionPrompt(msg("bossPhase3"),90); }
  ...
  if(enemies.length>0 && enemies.every(e=>!e.alive) && !areaCleared){
    areaCleared=true; player.energy=clamp(player.energy+35,0,100); gainUlt(520, "chain");
    if(area<3){ showCenter(battleModeSource==="projectArea" ? (language==="en"?"Area clear. Move to the next Area.":"Area清理完成，前往下一个Area。") : msg("moveRight"),80); addText(W-145,H/2,"NEXT AREA →","#ffe066",true); }
    else {
      let reward=0;
      let expReward=500;
  ...
  if(areaCleared && area<3 && player.x>W-70){ if(battleModeSource==="projectArea") ensureProjectAreaRun().areasCleared=Math.max(ensureProjectAreaRun().areasCleared, area); area++; player.x=95; player.y=H/2+115; particles=[]; slashes=[]; texts=[]; spawnArea(); }
...
function enemyHit(e){
  if(dist(player.x,player.y,e.x,e.y)<(e.boss?105:78)&&player.inv<=0&&player.guardTimer<=0){
    ...
    const dmg = e.boss ? (e.phase===3?26:18) : (e.rage?14:10);
...
function enterTeamSetup(){
  gameMode = "team";
  teamSelectSlot = 0;
  // remove locked characters from team
  for(let i=0;i<team.length;i++){
    if(!owned[team[i]]) team[i] = 0;
  }
...
function updateTeam(){
  ...
  const listX=95, listY=350, cardW=210, cardStep=245, visibleW=930;
  ...
  if(clicked){
    for(let i=0;i<3;i++){
      if(inRect(95+i*260,155,220,130)) teamSelectSlot = i;
    }
    ...
    for(let i=0;i<roles.length;i++){
      ...
      if(inRect(x,y,cardW,120) && owned[i]){
        ...
        if(existingSlot >= 0){
          ...
          team[teamSelectSlot] = i;
          teamSelectSlot = Math.min(2, teamSelectSlot + 1);
        }
      }
    }
...
function drawTeam(){
  ...
  for(let i=0;i<3;i++){
    ...
    drawPortrait(x+18,y+15,70,95,r,false);
    ctx.fillStyle="#ffe066";
    ...
    ctx.fillStyle="#fff";
    ctx.font="bold 22px " + FONT_UI;
    ctx.fillText(roleName(roleIndex),x+105,y+58);
    ctx.fillStyle="rgba(255,255,255,.65)";
    ctx.font="14px " + FONT_UI;
    ctx.fillText(i===0?mt("teamLeader"):mt("teamSwitchable"),x+105,y+92);
  }

  ctx.fillStyle="rgba(255,255,255,.70)";
  ctx.font="bold 18px " + FONT_UI;
  ctx.fillText(ui("selectOperator"),95,330);

  const listX=95, listY=350, cardW=210, cardStep=245, visibleW=930, listH=120;
  const totalW = roles.length * cardStep;
  const maxScroll = Math.max(0, totalW - visibleW);
  teamRosterScrollX = clamp(teamRosterScrollX || 0, 0, maxScroll);
...
function updateSettings(){
  menuPulse++;
  if(clicked){
    if(inRect(70,138,150,46)){ settingsTab="graphics"; logoutConfirm=false; localDeleteConfirm=false; }
    else if(inRect(235,138,150,46)){ settingsTab="audio"; logoutConfirm=false; localDeleteConfirm=false; }
    else if(inRect(400,138,150,46)){ settingsTab="account"; logoutConfirm=false; localDeleteConfirm=false; }

    if(inRect(60,560,220,52)){ enterLobby(); }
    if(inRect(310,560,220,52)){ returnToLogin(); }
    ...
    if(settingsTab==="graphics"){
      ...
      if(inRect(120,465,220,42)){ language = language==="zh" ? "en" : "zh"; rebuildStoryScripts(); refreshLanguageRuntimeText(); checkAchievements(); saveGame(); clicked=false; return; }
    }

    if(settingsTab==="audio"){
      if(inRect(120,245,260,52)){ audioMuted=!audioMuted; saveGame(); }
    }

    if(settingsTab==="account"){
      if(inRect(120,385,260,46)){
      ...
      if(!cloudUser && inRect(720,305,170,46) && promptAccountCredentials("login")){ accountLoginFlow(); }
      if(!cloudUser && inRect(910,305,170,46)){ bindCurrentLocalSaveToNewAccount(); }
...
function drawSettings(){
  ...
  if(settingsTab==="account"){
    ...
    ctx.fillText((language==="en"?"Protagonist Lv.":"主角 Lv.")+protagonistLevel()+"   "+protagonistSyncStatusText(),120,332);
    ...
    if(cloudUser){
      ...
      drawBtn(language==="en" ? "Request Deletion" : "申请注销账号","20 days",910,435,190,46,false,"#ff5555");
      ctx.fillStyle="rgba(255,255,255,.42)";
      ctx.font="13px " + FONT_UI;
      ctx.fillText(language==="en" ? "Sign Out clears local cache. Cloud save remains." : "退出登录会清除本地缓存；云端存档保留。",720,520);
      ctx.fillText(language==="en" ? "Deletion can be cancelled only by logging in again during the 20-day grace period." : "注销申请只能在20天内重新登录该账号时取消。",720,545);
    }else{
  ...
  drawBtn(ui("backLobby"),"ESC",60,560,220,52);
  drawBtn(ui("backLogin"),"",310,560,220,52);
...
function updateMail(){
  ...
  if(clicked){
    if(inRect(60,560,220,52)) enterLobby();
    if(inRect(120,210,760,110)){
      if(!mailClaimed){
        sfx("reward"); crystals += 300;
        gold += 1000;
        expBooks += 5;
        sfx("buy"); mailClaimed = true; saveGame(); autoCloudSaveNow(true);
        mailMsg = msg("mailClaimSuccess");
      } else {
...
function drawProfile(){
  ...
  ctx.font="bold 34px " + FONT_UI;
  ctx.textAlign="left";
  ctx.fillText(tr(ui("profile"),"Profile"),58,78);
  ctx.font="14px " + FONT_UI;
  ctx.fillStyle="rgba(255,255,255,.62)";
  ctx.fillText(tr("资料 / 点击任意位置或 Esc 返回","Profile / Click anywhere or Esc to return"),60,101);

  const cardX=150, cardY=165, cardW=820, cardH=315;
  ctx.fillStyle="rgba(255,255,255,.075)";
  ctx.fillRect(cardX,cardY,cardW,cardH);
  ctx.strokeStyle="rgba(124,199,255,.35)";
  ctx.lineWidth=2;
  ...
  ctx.fillStyle="#7cc7ff";
  ctx.beginPath();
  ctx.arc(cardX+100,cardY+110,55,0,Math.PI*2);
  ctx.fill();
  ctx.restore();
  ...
  ctx.textAlign="center";
  ctx.fillText((playerName||"P").slice(0,1).toUpperCase(),cardX+100,cardY+126);

  ctx.textAlign="left";
  ctx.fillStyle="#fff";
  ctx.font="bold 34px " + FONT_UI;
  ctx.fillText(playerName || "PLAYER", cardX+200, cardY+76);

  ctx.fillStyle="rgba(255,255,255,.66)";
  ctx.font="16px " + FONT_UI;
  ctx.fillText("UID: " + (playerUID || "--------"), cardX+200, cardY+112);

  ctx.fillStyle="rgba(255,255,255,.10)";
  ctx.fillRect(cardX+200,cardY+140,560,1);

  ctx.fillStyle="#ffe066";
  ctx.font="bold 22px " + FONT_UI;
  ctx.fillText(tr(ui("level"),"Level"), cardX+200, cardY+192);
  ctx.fillStyle="#fff";
  ctx.font="bold 28px " + FONT_UI;
  ctx.fillText("Lv." + protagonistLevel(), cardX+315, cardY+194);

  ctx.fillStyle="#7cc7ff";
  ctx.font="bold 22px " + FONT_UI;
  ctx.fillText(tr(ui("exp"),"EXP"), cardX+200, cardY+245);
  ctx.fillStyle="#fff";
  ctx.font="bold 24px " + FONT_UI;
  ctx.fillText(playerExp + " / " + playerExpNeed, cardX+315, cardY+246);

  ctx.fillStyle="#d66bff";
  ctx.font="bold 22px " + FONT_UI;
  ctx.fillText(tr(ui("progress"),"Progress"), cardX+200, cardY+298);
  ctx.fillStyle="#fff";
  ctx.font="bold 24px " + FONT_UI;
  ctx.fillText(getProfileProgressText(), cardX+315, cardY+299);
}
...
function drawWarehouse(){
  ...
  ctx.font="bold 34px " + FONT_UI;
  ctx.textAlign="left";
  ctx.fillText(ui("warehouse"),58,78);
  ctx.font="14px " + FONT_UI;
  ctx.fillStyle="rgba(255,255,255,.62)";
  ctx.fillText("Inventory / Materials",60,101);

  const items=[
    [ui("crystal"),crystals,tx("operatorBuyShop")],
    [tr("金币","Gold"),gold,tx("upgradeWeaponHint")],
    [tr("经验书","EXP Books"),expBooks,tx("characterLevelMat")],
    [tr("精炼合金","Weapon Ore"),weaponOre,tx("weaponUpgradeMat")]
  ];

  for(let i=0;i<items.length;i++){
    const x=80+(i%2)*430;
    const y=170+Math.floor(i/2)*160;
    ctx.fillStyle="rgba(255,255,255,.075)";
    ctx.fillRect(x,y,360,120);
    ctx.strokeStyle="rgba(255,255,255,.14)";
    ctx.strokeRect(x,y,360,120);

    ctx.fillStyle=i===0?"#7cc7ff":i===1?"#ffe066":i===2?"#ffffff":"#c35cff";
    ctx.font="bold 28px " + FONT_UI;
    ctx.fillText(items[i][0],x+26,y+42);
    ctx.fillStyle="#fff";
    ctx.font="bold 30px " + FONT_UI;
    ctx.textAlign="right";
    ctx.fillText(String(items[i][1]),x+330,y+58);
    ctx.textAlign="left";
    ctx.fillStyle="rgba(255,255,255,.62)";
    ctx.font="14px " + FONT_UI;
    ctx.fillText(items[i][2],x+26,y+90);
  }

  ctx.fillStyle="rgba(0,0,0,.32)";
  ctx.fillRect(80,500,800,44);
  ctx.strokeStyle="rgba(255,255,255,.12)";
  ctx.strokeRect(80,500,800,44);
  ctx.fillStyle="rgba(255,255,255,.78)";
  ctx.font="15px " + FONT_UI;
  ctx.fillText(tx("tipPrefix")+warehouseMsg,100,528);
...
function updateEvent(){
  ...
  if(clicked){
    ...
    // left event list
    if(inRect(38,150,185,58)) eventTab="login";
    else if(inRect(38,220,185,58)) eventTab="level";
    else if(inRect(38,290,185,58)) eventTab="supply";
...
function drawEvent(){
  ...
  ctx.fillStyle="rgba(124,199,255,.08)";
  ctx.beginPath();
  ctx.moveTo(260,80);
  ctx.lineTo(840,65);
  ctx.lineTo(1035,590);
  ...
  for(let i=0;i<list.length;i++){
    ...
    ctx.strokeStyle=active?"rgba(255,224,102,.75)":"rgba(255,255,255,.14)";
    ctx.strokeRect(38,y,185,58);
    ctx.fillStyle=active?"#ffe066":"#fff";
    ctx.font="bold 20px " + FONT_UI;
  ...
  ctx.strokeStyle="rgba(124,199,255,.24)";
  ...
  ctx.fillStyle="#fff";
  ctx.font="bold 30px " + FONT_UI;
  const title = eventTab==="login" ? ui("loginEvent") : eventTab==="level" ? tx("rookieGrowth") : ui("supplyEvent");
  ...
  ctx.fillText(eventTab==="login"?mt("eventPanelLoginTitle"):eventTab==="level"?mt("eventPanelLevelTitle"):mt("eventPanelSupplyTitle"),panelX+25,panelY+40);
  ctx.fillStyle="rgba(255,255,255,.58)";
  ctx.font="13px " + FONT_UI;
  ctx.fillText(eventMsg || (eventTab==="login" ? (canClaimDailyLogin() ? mt("dailyCheckAvailable") : mt("claimedTodayComeTomorrow")) : mt("eventDefaultHint")),panelX+25,panelY+64);
...
function drawLogin(){
  ...
  ctx.fillText(accountPassword ? "●".repeat(Math.min(16,accountPassword.length)) : accTx("enterPassword"), panelX+60, panelY+204);
...
function drawLobbyPlaceholderRole(i){
  ...
  ctx.font="bold 28px " + FONT_UI;
  ctx.textAlign="center";
  ctx.fillText(roleName(i),0,166);
  ctx.fillStyle="rgba(255,255,255,.68)";
  ctx.font="16px " + FONT_UI;
  ctx.fillText(roleStyle(i),0,196);
  ctx.restore();
}
...
const GROWTH_GUIDE_PAGES = [
  {title:{zh:"初入雷文哈多",en:"Arrival in Ravenhado"}, pageReward:{crystal:200,gold:5000,books:3,ore:1}, tasks:[
    {zh:"完成主线 01-01",en:"Clear Main 01-01", check:()=>!!cleared[1], reward:{exp:500,gold:1000,books:1}},
  ...
  {title:{zh:"稳定战线",en:"Stable Frontline"}, pageReward:{crystal:300,gold:7000,books:5,ore:2}, tasks:[
    {zh:"完成主线 01-03",en:"Clear Main 01-03", check:()=>!!cleared[3], reward:{exp:500,gold:1000,books:1}},
  ...
  {title:{zh:"武装检查",en:"Armament Check"}, pageReward:{crystal:350,gold:8000,books:5,ore:2}, tasks:[
    {zh:"完成主线 01-05",en:"Clear Main 01-05", check:()=>!!cleared[5], reward:{exp:500,gold:1000,books:1}},
  ...
  {title:{zh:"突破准备",en:"Breakthrough Prep"}, pageReward:{crystal:400,gold:10000,books:7,ore:3}, tasks:[
    {zh:"完成主线 01-07",en:"Clear Main 01-07", check:()=>!!cleared[7], reward:{exp:500,gold:1000,books:1}},
  ...
  {title:{zh:"高阶训练",en:"Advanced Training"}, pageReward:{crystal:500,gold:12000,books:9,ore:4}, tasks:[
    {zh:"完成主线 01-09",en:"Clear Main 01-09", check:()=>!!cleared[9], reward:{exp:500,gold:1000,books:1}},
  ...
  {title:{zh:"旅途起点",en:"Journey Begins"}, pageReward:{crystal:1000,gold:20000,books:15,ore:6}, tasks:[
    {zh:"完成主线 01-11",en:"Clear Main 01-11", check:()=>!!cleared[11], reward:{exp:500,gold:1200,books:1}},
...
function applyGrowthReward(r){
  ...
  expBooks += r.books||0;
  weaponOre += r.ore||0;
  if(r.crystal){ crystals += r.crystal; totalCrystalsEarned += r.crystal; }
}
...
function growthRewardText(r){
  ...
  if(r.crystal) parts.push((language==="en"?"Crystal ":"水晶 ")+r.crystal);
  return parts.join("   ");
}
...
const ACTION_RECORD_LEVELS_PER_PAGE = 10;
...
function arTaskList(){
  return [
    {id:"login", zh:"登录游戏", en:"Log in", need:1, exp:80},
    {id:"battle", zh:"完成任意关卡", en:"Clear any stage", need:1, exp:150},
    {id:"daydream", zh:"完成一次白日梦", en:"Complete one Daydream", need:1, exp:180},
    {id:"patrol", zh:"领取一次巡逻", en:"Claim one Patrol", need:1, exp:120}
  ];
}
function arTaskProgress(id){ arEnsureTaskState(); return actionRecordTaskState.done[id] || 0; }
function arTaskReady(t){ return arTaskProgress(t.id) >= t.need && !actionRecordTaskState.claimed[t.id]; }
function arTaskClaim(t){
  arEnsureTaskState();
  if(!arTaskReady(t)) return;
  actionRecordTaskState.claimed[t.id] = true;
  addActionRecordExp(t.exp, t.id);
  arSetMsg((language==="en"?"Task EXP +":"任务经验 +") + t.exp, 80);
  sfx("reward");
  saveGame();
}
...
function arGrantReward(track, lv){
  ...
  if(r.kind==="crystal"){ crystals += r.amount; totalCrystalsEarned += r.amount; }
...
function buyActionRecordAdvanced(){
  ...
  if(crystals < ACTION_RECORD_ADVANCED_PRICE){ arSetMsg(arText("水晶不足","Not enough Crystal"),70); return; }
...
function drawActionRecordMiniButton(){
  ...
  if(hover){
    ...
    ctx.fillStyle="#bfe8ff"; ctx.font="bold 13px "+FONT_UI; ctx.fillText(arText("行动记录","Action Record"),W-185,74);
  }
  ctx.restore();
}
function drawArRewardBox(track, lv, x, y, w, h){
  ...
  ctx.save();
  ctx.fillStyle = claimed ? "rgba(124,199,255,.12)" : can ? "rgba(255,224,102,.12)" : reached && unlocked ? "rgba(255,255,255,.075)" : "rgba(255,255,255,.035)";
  ctx.fillRect(x,y,w,h);
  ctx.strokeStyle = can ? "rgba(255,224,102,.62)" : claimed ? "rgba(124,199,255,.48)" : "rgba(255,255,255,.12)";
  ctx.strokeRect(x,y,w,h);
  ctx.fillStyle = claimed ? "#7cc7ff" : can ? "#ffe066" : reached && unlocked ? "#ffffff" : "rgba(255,255,255,.38)";
  ctx.font="bold 12px "+FONT_UI; ctx.textAlign="center";
  const label=arRewardLabel(r);
  fitText(label,w-12,12,"bold",9);
  ctx.fillText(label,x+w/2,y+33);
  ctx.font="11px "+FONT_UI;
  ctx.fillStyle = claimed ? "#7cc7ff" : can ? "#ffe066" : "rgba(255,255,255,.42)";
  ctx.fillText(claimed ? "✓" : can ? arText("可领取","Claim") : (!unlocked?arText("未解锁","Locked"):reached?arText("未领取","Ready"):"Lv."+lv),x+w/2,y+h-13);
  ctx.restore();
}
function drawActionRecord(){
  arEnsureTaskState();
  const bg=ctx.createLinearGradient(0,0,W,H); bg.addColorStop(0,"#0b1220"); bg.addColorStop(.52,"#060912"); bg.addColorStop(1,"#02040a");
  ctx.fillStyle=bg; ctx.fillRect(0,0,W,H);
  ctx.fillStyle="rgba(255,255,255,.035)"; ctx.fillRect(28,24,1064,612); ctx.strokeStyle="rgba(124,199,255,.18)"; ctx.strokeRect(28,24,1064,612);

  // Endfield-style fixed left information area.
  ctx.fillStyle="rgba(0,0,0,.30)"; ctx.fillRect(56,54,265,552); ctx.strokeStyle="rgba(124,199,255,.28)"; ctx.strokeRect(56,54,265,552);
  ctx.fillStyle="rgba(124,199,255,.12)"; ctx.fillRect(56,54,265,4);
  ctx.fillStyle="#fff"; ctx.font="bold 31px "+FONT_UI; ctx.textAlign="left"; ctx.fillText(arText("行动记录","Action Record"),82,104);
  ctx.fillStyle="rgba(255,255,255,.45)"; ctx.font="12px "+FONT_UI; ctx.fillText(arText("事务处任务记录","Office Task Record"),84,128);
  ctx.fillStyle="#bfe8ff"; ctx.font="bold 72px "+FONT_UI; ctx.fillText(String(actionRecordLevel).padStart(2,"0"),84,218);
  ctx.fillStyle="rgba(255,255,255,.62)"; ctx.font="13px "+FONT_UI; ctx.fillText(arText("行动记录等级","Action Record Level"),88,242);
  ctx.fillStyle="rgba(255,255,255,.12)"; ctx.fillRect(84,266,204,10);
  ctx.fillStyle="#7cc7ff"; ctx.fillRect(84,266,204*clamp(actionRecordExp/actionRecordExpNeed,0,1),10);
  ctx.fillStyle="rgba(255,255,255,.70)"; ctx.font="13px "+FONT_UI; ctx.fillText(actionRecordExp+" / "+actionRecordExpNeed+" EXP",84,298);
  ctx.fillStyle="rgba(255,255,255,.46)"; ctx.font="12px "+FONT_UI; wrapText(arText("完成每日任务与关卡获得经验。该等级独立于主角和执行官等级。","Earn EXP from tasks and stages. This level is independent from characters."),84,326,206,19);

  drawBtn(actionRecordAdvanced||actionRecordUltimate?arText("高级已解锁","Advanced Unlocked"):arText("高级 1500 水晶","Advanced 1500 Crystal"),"",82,400,206,40,!(actionRecordAdvanced||actionRecordUltimate),"#7cc7ff");
  drawBtn(actionRecordUltimate?arText("顶级已解锁","Ultimate Unlocked"):arText("顶级 "+ACTION_RECORD_ULTIMATE_PRICE_TEXT,"Ultimate "+ACTION_RECORD_ULTIMATE_PRICE_TEXT),"",82,448,206,40,!actionRecordUltimate,"#ffe066");
  ctx.fillStyle="rgba(255,255,255,.48)"; ctx.font="12px "+FONT_UI; wrapText(arText("高级返还 1800 水晶。顶级包含 4000 水晶与雷文哈多武器库完整权限。","Advanced returns 1800 Crystal. Ultimate includes 4000 Crystal and full Ravenhado Armory access."),84,514,206,18);
  drawBtn(arText("返回","Back"),"ESC",82,560,206,38,true,"#fff");

  // Right reward track area.
  ...
  ctx.fillStyle="#fff"; ctx.font="bold 24px "+FONT_UI; ctx.textAlign="left"; ctx.fillText(arText("奖励轨道","Reward Track"),352,76);
  ctx.fillStyle="rgba(255,255,255,.46)"; ctx.font="12px "+FONT_UI; ctx.fillText("Lv."+start+" - Lv."+end+"  /  "+arText("横向节点","Horizontal Track"),354,100);
  drawBtn("<","",922,58,46,32,actionRecordPage>0,"#fff"); drawBtn(">","",978,58,46,32,actionRecordPage<4,"#fff");

  const x0=354, y0=138, colW=66, gap=8, boxH=62;
  for(let i=0;i<ACTION_RECORD_LEVELS_PER_PAGE;i++){
    ...
    if(i<ACTION_RECORD_LEVELS_PER_PAGE-1){ ctx.strokeStyle=reached?"rgba(124,199,255,.78)":"rgba(255,255,255,.14)"; ctx.lineWidth=2; ctx.beginPath(); ctx.moveTo(x+colW/2+9,120); ctx.lineTo(x+colW+gap+colW/2-9,120); ctx.stroke(); }
    ctx.fillStyle=current?"#ffe066":reached?"#7cc7ff":"rgba(255,255,255,.28)";
    ctx.beginPath(); ctx.arc(x+colW/2,120,current?8:5,0,Math.PI*2); ctx.fill();
    ctx.fillStyle="rgba(255,255,255,.66)"; ctx.font="11px "+FONT_UI; ctx.textAlign="center"; ctx.fillText(String(lv),x+colW/2,103);
  }

  const tracks=["free","advanced","ultimate"];
  for(let row=0; row<tracks.length; row++){
    const track=tracks[row], y=y0+row*98;
    ctx.fillStyle=track==="free"?"#d8e1ec":track==="advanced"?"#7cc7ff":"#ffe066";
    ctx.font="bold 14px "+FONT_UI; ctx.textAlign="left"; ctx.fillText(arTrackName(track),354,y-12);
    ctx.strokeStyle="rgba(255,255,255,.09)"; ctx.beginPath(); ctx.moveTo(354,y-4); ctx.lineTo(1030,y-4); ctx.stroke();
    for(let i=0;i<ACTION_RECORD_LEVELS_PER_PAGE;i++){
      const lv=start+i; if(lv>ACTION_RECORD_MAX_LEVEL) continue;
      drawArRewardBox(track,lv,x0+i*(colW+gap),y,colW,boxH);
    }
  }

  // Simple task panel.
  const ty=466;
  ctx.fillStyle="rgba(0,0,0,.22)"; ctx.fillRect(354,ty,676,132); ctx.strokeStyle="rgba(124,199,255,.20)"; ctx.strokeRect(354,ty,676,132);
  ctx.fillStyle="#fff"; ctx.font="bold 16px "+FONT_UI; ctx.textAlign="left"; ctx.fillText(arText("简单任务","Simple Tasks"),372,492);
  const tasks=arTaskList();
  for(let i=0;i<tasks.length;i++){
    const t=tasks[i], x=372+i*162, y=510, prog=Math.min(t.need, arTaskProgress(t.id));
    const ready=arTaskReady(t), claimed=!!actionRecordTaskState.claimed[t.id];
    ctx.fillStyle=claimed?"rgba(124,199,255,.10)":ready?"rgba(255,224,102,.12)":"rgba(255,255,255,.05)"; ctx.fillRect(x,y,148,66);
    ctx.strokeStyle=ready?"rgba(255,224,102,.55)":claimed?"rgba(124,199,255,.40)":"rgba(255,255,255,.11)"; ctx.strokeRect(x,y,148,66);
    ctx.fillStyle=claimed?"#7cc7ff":ready?"#ffe066":"#fff"; ctx.font="bold 12px "+FONT_UI; ctx.textAlign="left"; ctx.fillText(language==="en"?t.en:t.zh,x+10,y+22);
    ctx.fillStyle="rgba(255,255,255,.55)"; ctx.font="11px "+FONT_UI; ctx.fillText(prog+"/"+t.need+"   EXP +"+t.exp,x+10,y+42);
    ctx.fillStyle=claimed?"#7cc7ff":ready?"#ffe066":"rgba(255,255,255,.38)"; ctx.textAlign="right"; ctx.fillText(claimed?"✓":ready?arText("领取","Claim"):"—",x+136,y+58);
  }

  if(actionRecordMsgTimer>0){
    ctx.fillStyle="rgba(0,0,0,.45)"; ctx.fillRect(354,606,676,24); ctx.strokeStyle="rgba(255,255,255,.12)"; ctx.strokeRect(354,606,676,24);
    ctx.fillStyle="#ffe066"; ctx.font="bold 13px "+FONT_UI; ctx.textAlign="left"; ctx.fillText(actionRecordMsg,370,623);
  }
  if(actionRecordWeaponSelecting) drawActionRecordWeaponSelect();
}
function drawActionRecordWeaponSelect(){
  const list=arArmoryList();
  const x=W/2-260,y=H/2-175,w=520,h=350;
  ctx.save(); ctx.fillStyle="rgba(0,0,0,.58)"; ctx.fillRect(0,0,W,H);
  ctx.fillStyle="rgba(7,10,18,.97)"; ctx.fillRect(x,y,w,h); ctx.strokeStyle="rgba(124,199,255,.56)"; ctx.lineWidth=2; ctx.strokeRect(x,y,w,h);
  ...
  for(let i=0;i<list.length;i++){
    const yy=y+112+i*54, sel=i===actionRecordSelectedWeapon, item=list[i];
    ctx.fillStyle=sel?"rgba(124,199,255,.16)":"rgba(255,255,255,.055)"; ctx.fillRect(x+38,yy,444,42); ctx.strokeStyle=sel?"rgba(124,199,255,.65)":"rgba(255,255,255,.12)"; ctx.strokeRect(x+38,yy,444,42);
    ctx.fillStyle=sel?"#bfe8ff":"#fff"; ctx.font="bold 15px "+FONT_UI; ctx.fillText((sel?"● ":"○ ")+(language==="en"?item.en:item.zh)+"（"+(language==="en"?item.typeEn:item.typeZh)+"）",x+58,yy+27);
  }
  drawBtn(arText("确认","Confirm"),"",x+138,y+h-64,116,42,true,"#ffe066");
  drawBtn(arText("取消","Cancel"),"",x+278,y+h-64,116,42,true,"#fff");
  ctx.restore();
}
function updateActionRecord(){
  ...
  if(inRect(82,560,206,38)){ gameMode="lobby"; clicked=false; return; }
  if(inRect(82,400,206,40)){ buyActionRecordAdvanced(); clicked=false; return; }
  if(inRect(82,448,206,40)){ buyActionRecordUltimate(); clicked=false; return; }
  if(inRect(922,58,46,32) && actionRecordPage>0){ actionRecordPage--; clicked=false; return; }
  if(inRect(978,58,46,32) && actionRecordPage<4){ actionRecordPage++; clicked=false; return; }
  const x0=354, y0=138, colW=66, gap=8, boxH=62, startLv=actionRecordPage*ACTION_RECORD_LEVELS_PER_PAGE+1;
  const tracks=["free","advanced","ultimate"];
  for(let row=0; row<tracks.length; row++){
    const y=y0+row*98;
    for(let i=0;i<ACTION_RECORD_LEVELS_PER_PAGE;i++){
      const lv=startLv+i; if(lv>ACTION_RECORD_MAX_LEVEL) continue;
      const x=x0+i*(colW+gap);
      if(inRect(x,y,colW,boxH)){ arClaim(tracks[row], lv); clicked=false; return; }
    }
  }
  const tasks=arTaskList();
  for(let i=0;i<tasks.length;i++){
    const x=372+i*162, y=510;
    if(inRect(x,y,148,66)){ arTaskClaim(tasks[i]); clicked=false; return; }
  }
  clicked=false;
}
...
function drawGrowthGuide(){
  ...
  drawBtn(language==="en"?"Daily Tasks":"每日任务","",256,104,170,44,false,"#777");
...
function drawLobbySwitchButton(){
  ...
  ctx.lineCap="round";
  ctx.beginPath();
  ctx.arc(x,y,8,-0.2,Math.PI*1.25);
  ctx.stroke();
...
function drawLobby(){
  ...
  ctx.fillStyle="rgba(80,110,200,.08)";
  ctx.beginPath();
  ctx.moveTo(150,115);
  ctx.lineTo(820,115);
  ctx.lineTo(970,H);
  ...
  ctx.fillStyle="rgba(255,255,255,.075)";
  ctx.fillRect(28,30,360,116);
  ctx.strokeStyle="rgba(255,255,255,.13)";
  ctx.lineWidth=1;
  ctx.strokeRect(28,30,360,116);
  ...
  ctx.font="bold 23px " + FONT_UI;
  ctx.textAlign="center";
  ctx.fillText((playerName||"P").slice(0,1).toUpperCase(),67,82);

  ctx.textAlign="left";
  ctx.fillStyle="#fff";
  ctx.font="bold 27px " + FONT_UI;
  ctx.fillText(playerName || "PLAYER",105,64);

  ctx.fillStyle="rgba(255,255,255,.58)";
  ctx.font="12px " + FONT_UI;
  ctx.fillText("UID: "+(playerUID||"--------"),105,88);

  ctx.fillStyle="rgba(255,255,255,.78)";
  ctx.font="14px " + FONT_UI;
  ctx.fillText("Lv."+playerLevel,105,113);

  const ex = 160, ey = 103, ew = 185, eh = 9;
  ctx.fillStyle="rgba(255,255,255,.14)";
  ctx.fillRect(ex,ey,ew,eh);
  ctx.fillStyle="#7cc7ff";
  ctx.fillRect(ex,ey,ew*clamp(playerExp/playerExpNeed,0,1),eh);
  ctx.strokeStyle="rgba(255,255,255,.18)";
  ctx.strokeRect(ex,ey,ew,eh);
  ctx.fillStyle="rgba(255,255,255,.68)";
  ctx.font="12px " + FONT_UI;
  ctx.fillText("EXP "+playerExp+"/"+playerExpNeed, ex, 128);

  // compact resource bar: Crystal / Gold / Stamina
  const cbx = 405, cby = 30, cbw = 500, cbh = 46;
  drawCompactResourceBar(cbx,cby,cbw);

  // center lobby executor display
  drawLobbyExecutor();
  ...
  // right menu
  const px=W-255;
  drawBtn(tr(ui("operation"),"Operation"),"GO",px+22,118,208,54,true,"#ffe066");
  drawBtn(tr(ui("operators"),"Executors"),"",px+22,188,208,54);
  drawBtn(tr(ui("shop"),"Shop"),"",px+22,258,208,54,false,"#7cc7ff");
  drawBtn(tr(ui("mail"),"Mail"),mailClaimed?tr(ui("claimed"),"Claimed"):"NEW",px+22,328,208,54);
  drawBtn(tr(ui("event"),"Event"),"NEW",px+22,398,208,54,false,"#d66bff");
  drawBtn(tr(ui("warehouse"),"Inventory"),"",px+22,468,208,54);
  drawBtn(tr(ui("settings"),"Settings"),"",px+22,538,208,54,false,"#7cc7ff");

  // notice
  ctx.fillStyle="rgba(0,0,0,.34)";
  ctx.fillRect(38,573,650,50);
  ctx.strokeStyle="rgba(255,255,255,.10)";
  ctx.strokeRect(38,573,650,50);
  ctx.fillStyle="rgba(255,255,255,.82)";
  ctx.font="bold 15px " + FONT_UI;
  ctx.textAlign="left";
  fitText(tr("公告：","Notice: ")+notice,610,15,"bold",11);
  ctx.fillText(tr("公告：","Notice: ")+notice,58,604);

  // Draw achievement button LAST so it won't be covered by lobby panels

  // Top-right small achievement icon, drawn last to avoid being covered

  // Top-right small achievement icon, drawn last to avoid being covered
  checkAchievementsThrottled();
  drawActionRecordMiniButton();
  drawGrowthGuideMiniButton();
  drawAchievementMiniButton();

  if(prologueDone && !lobbyGuideDone){
    ctx.fillStyle="rgba(0,0,0,.66)";
    ctx.fillRect(425,505,410,72);
    ctx.strokeStyle="rgba(255,224,102,.45)";
    ctx.strokeRect(425,505,410,72);
    ctx.fillStyle="#ffe066";
    ctx.font="bold 18px " + FONT_UI;
    ctx.textAlign="left";
    ctx.fillText(tx("protagonist"),445,532);
    ctx.fillStyle="#fff";
    ctx.font="16px " + FONT_UI;
    ctx.fillText(msg("lobbyGuide"),445,558);
  }
...
const commissionStages = [
  {id:1,name:"C-01 Street Sweep",zh:"委托01 街区清理",desc:"Clear crystal monsters near the outer street.",zhDesc:"清理外围街区的晶体怪。",reward:50,exp:500,type:"annihilation",lv:1,boss:false},
  {id:2,name:"C-02 East Patrol",zh:"委托02 东区巡逻",desc:"Patrol the east route and remove threats.",zhDesc:"巡逻东区路线并清除威胁。",reward:50,exp:500,type:"annihilation",lv:2,boss:false},
  {id:3,name:"C-03 Warehouse Alert",zh:"委托03 仓库警报",desc:"Shielded enemies detected inside the warehouse.",zhDesc:"仓库内发现带盾晶体怪。",reward:50,exp:500,type:"shield",lv:3,boss:false},
  {id:4,name:"C-04 Station Suppression",zh:"委托04 车站压制",desc:"Ranged enemies are gathering near the station.",zhDesc:"远程晶体怪正在车站区域聚集。",reward:50,exp:500,type:"ranged",lv:4,boss:false},
  {id:5,name:"C-05 Blockade Line",zh:"委托05 封锁线",desc:"Break through the blockade line.",zhDesc:"突破被晶体怪占据的封锁线。",reward:50,exp:500,type:"elite",lv:5,boss:false},
  {id:6,name:"C-06 Rift Square",zh:"委托06 裂晶广场",desc:"Survive and stabilize the square.",zhDesc:"在裂晶广场坚持并稳定局势。",reward:50,exp:500,type:"survival",lv:6,boss:false},
  {id:7,name:"C-07 Underground Route",zh:"委托07 地下通道",desc:"Multiple enemy waves in the underground route.",zhDesc:"地下通道出现多波敌人。",reward:50,exp:500,type:"mixed",lv:7,boss:false},
  {id:8,name:"C-08 Crystal Lab",zh:"委托08 晶体实验区",desc:"High-risk enemies detected in the lab area.",zhDesc:"实验区出现高危晶体怪。",reward:50,exp:500,type:"mixed",lv:8,boss:false},
  {id:9,name:"C-09 Core Perimeter",zh:"委托09 核心外围",desc:"Elite enemies are guarding the core perimeter.",zhDesc:"精英晶体怪正在守卫核心外围。",reward:50,exp:500,type:"bossPrep",lv:9,boss:false},
  {id:10,name:"C-10 Core Guardian",zh:"委托10 晶核守卫",desc:"Defeat the core guardian.",zhDesc:"击败晶核守卫。",reward:50,exp:500,type:"chapterBoss",lv:10,boss:true}
];
...
function dungeonHomeCards(){
  return language === "en" ? [
    {key:"material", name:"Material Dungeon", icon:"◆", color:"#7cc7ff", desc:"Gold / character upgrade / weapon materials."},
    {key:"boss", name:"Boss Challenge", icon:"B", color:"#ff6b9b", desc:"Powerful enemies. Coming soon."},
    {key:"explore", name:"Exploration Map", icon:"◇", color:"#7cffb2", desc:"Chests / crates / one-time rewards. Coming soon."},
...
function materialDungeonsV42(){
  return language === "en" ? [
    ...
    {key:"exp", name:"Character Material", short:"Character", color:"#7cffb2", desc:"Obtain character upgrade materials.", enemy:"Crystal Beast", base:{gold:650, expBooks:4, weaponOre:0}},
    {key:"weapon", name:"Weapon Material", short:"Weapon", color:"#7cc7ff", desc:"Obtain weapon upgrade materials.", enemy:"Crystal Guard", base:{gold:650, expBooks:0, weaponOre:4}}
  ] : [
...
function normalizeDungeonRuntime(){
  ...
  materialDungeonDifficulty = clamp(Math.floor(materialDungeonDifficulty || 1), 1, 6);
  materialDungeonSelected = clamp(Math.floor(materialDungeonSelected || 0), 0, 2);
  if(!dungeonPanelMode) dungeonPanelMode = "home";
}
...
function drawDungeonTopBarV42(){
  ...
  ctx.font="bold 15px " + FONT_UI;
  ctx.textAlign="left";
  ctx.fillText((language==="en"?"Stamina ":"体力 ")+Math.floor(dungeonStamina)+"/240",W-392,61);
  ctx.fillStyle="#7cc7ff";
...
function drawMaterialDungeonDetailV42(){
  ...
  // stage type cards
  for(let i=0;i<list.length;i++){
    const d=list[i], x=95+i*225, y=245, w=205, h=76;
    const sel=materialDungeonSelected===i;
    ctx.fillStyle=sel?"rgba(255,224,102,.18)":"rgba(255,255,255,.055)";
    ctx.fillRect(x,y,w,h);
  ...
  for(let i=1;i<=6;i++){
    ...
    const sel=materialDungeonDifficulty===i;
    ctx.fillStyle=sel?"rgba(255,128,64,.24)":"rgba(255,255,255,.055)";
    ctx.fillRect(x,y,w,h);
    ctx.strokeStyle=sel?"#ff9955":"rgba(255,255,255,.16)";
    ctx.strokeRect(x,y,w,h);
    ctx.fillStyle=sel?"#ff9955":"#fff";
    ctx.font="bold 22px Arial";
    ctx.textAlign="center";
    ctx.fillText(roman(i),x+w/2,y+30);
  }
  ...
  drawBtn(language==="en"?"Start":"开始",(language==="en"?"Team":"编队"),830,505,180,44,dungeonStamina>=r.staminaCost,dungeonStamina>=r.staminaCost?"#ffe066":"#888");
}
...
function applyProjectAreaReward(){
  ...
  if(reward.crystal>0){
    crystals += reward.crystal;
    totalCrystalsEarned += reward.crystal;
  }
...
function drawDungeonInlinePanel(){
  ...
  else if(dungeonPanelMode==="patrol" && window.PZPatrol) window.PZPatrol.drawDetail();
  else drawDungeonHomeV42();
...
function drawBossKrosDetail(){
  ...
  ctx.fillStyle=unlocked?"rgba(255,107,155,.12)":"rgba(255,255,255,.045)";
...
function updateDungeonInlineClicks(){
  ...
  if(dungeonPanelMode==="patrol" && window.PZPatrol){
    return window.PZPatrol.handleDungeonClick();
  }
  ...
  if(dungeonPanelMode==="material"){
    ...
    for(let i=1;i<=6;i++){
      if(inRect(95+(i-1)*58,368,48,45)){
        const unlockLv = [0,1,20,40,60,80,100][i] || 1;
        if(playerLevel < unlockLv){
          showCenter((language==="en"?"Requires Lv.":"需要玩家等级 ")+unlockLv,70);
        }else{
          materialDungeonDifficulty=i;
        }
        clicked=false;
    ...
    if(inRect(830,505,180,44)){
      const unlockLv = [0,1,20,40,60,80,100][materialDungeonDifficulty] || 1;
      if(playerLevel < unlockLv) showCenter((language==="en"?"Requires Lv.":"需要玩家等级 ")+unlockLv,70);
      else startMaterialDungeonTeam();
      clicked=false;
      return true;
    }
    return false;
...
function updateStaminaRecoverOverlay(){
  ...
  if(clicked){
    ...
    if(inRect(x+530,y+14,34,34) || inRect(x+185,y+360,200,44)){
      closeStaminaRecover();
      return true;
    }
...
function drawStaminaRecoverOverlay(){
  ...
  ctx.fillStyle="rgba(255,255,255,.55)";
  ctx.font="14px " + FONT_UI;
  ctx.fillText(language==="en"?"Current Stamina":"当前体力",x+32,y+82);
...
function drawCompactResourceBar(x,y,w=500){
  normalizeDungeonRuntime();
  ctx.save();
  ...
  ctx.fillStyle="#7cc7ff";
  ctx.fillText(tr("◆ 水晶","◆ Crystal"), x+18, y+29);
  ctx.fillStyle="#ffffff";
  ctx.fillText(String(crystals), x+82, y+29);

  ctx.fillStyle="#ffe066";
  ctx.fillText(tr("● 金币","● Gold"), x+198, y+29);
  ctx.fillStyle="#ffffff";
  ctx.fillText(String(gold), x+260, y+29);

  ctx.fillStyle="#ffbf66";
  ctx.fillText("⚡", x+365, y+29);
  ctx.fillStyle="#ffffff";
...
function currentOperationStages(){
  if(selectedTab==="combat") return commissionStages;
  return stages;
}

function currentStageData(){
  const list = currentOperationStages();
  return list[Math.max(0, selectedStage-1)] || list[0];
}
...
function operationNodePos(i){
  ...
  const col = i % cols;
  return {x:125 + col*170, y:245 + row*145};
}
...
function drawOperation(){
  ...
  ctx.font="bold 34px " + FONT_UI;
  ctx.textAlign="left";
  ctx.fillText(selectedTab==="daydream" ? (language==="en"?"Daydream Reconstruction":"白日梦重现") : (selectedTab==="sideStory" ? "Side Story" : (selectedTab==="dungeon" ? (language==="en"?"Dungeon":"副本") : (selectedTab==="combat" ? (language==="en"?"Commission Operation":"委托作战") : tx("chapterTitle")))),58,78);
  ctx.font="14px " + FONT_UI;
  ctx.fillStyle="rgba(255,255,255,.62)";
  ctx.fillText(selectedTab==="daydream" ? (language==="en"?"Inspired by node-route roguelike structure; adapted for PZ story investigations.":"参考节点路线式肉鸽结构，改造成PZ剧情调查模式。") : (selectedTab==="sideStory" ? (language==="en"?"Coming soon.":"开发中。") : (selectedTab==="dungeon" ? (language==="en"?"Material / Boss / Project Area / Patrol.":"材料副本 / Boss / Project Area / 巡逻。") : (selectedTab==="combat" ? (language==="en"?"10 Commissions / Reward: 50 Crystal + 500 EXP each":"10个委托 / 每关奖励：50水晶 + 500EXP") : tx("operationSubtitle")+crystals))),60,102);
  ...
  for(let i=0;i<list.length;i++){
    ...
    const p=selectedTab==="combat" ? operationNodePos(i) : nodePos(i);
    const locked=selectedTab==="combat" ? false : (i>0&&!cleared[i]);
    const done=selectedTab==="combat" ? !!cleared["c"+st.id] : !!cleared[st.id];
    ...
    ctx.fill();
    ctx.strokeStyle=sel?"#ffe066":done?"#7cc7ff":locked?"#777":"#fff";
    ctx.lineWidth=sel?4:2;
    ctx.stroke();
    ctx.fillStyle=st.boss?"#ff6b9b":done?"#7cc7ff":locked?"#888":"#fff";
    ctx.font=st.boss?"bold 18px Arial":"bold 16px Arial";
    ctx.textAlign="center";
    ctx.fillText(st.boss?"B":String(st.id).padStart(2,"0"),p.x,p.y+6);
    ctx.font="12px " + FONT_UI;
    ctx.fillStyle="rgba(255,255,255,.70)";
  ...
  drawBtn(language==="en"?"Dungeon":"副本","",390,595,135,38,selectedTab==="dungeon",selectedTab==="dungeon"?"#ffe066":"#7cc7ff");
  drawBtn("Side Story","",540,595,135,38,selectedTab==="sideStory",selectedTab==="sideStory"?"#c35cff":"#fff");
  drawBtn(language==="en"?"Daydream":"白日梦","",690,595,135,38,selectedTab==="daydream",selectedTab==="daydream"?"#9b7cff":"#fff");
  ...
  if(selectedTab!=="dungeon" && selectedTab!=="sideStory" && selectedTab!=="daydream" && operationDetailVisible){
    ...
    ctx.font="bold 26px " + FONT_UI;
    ctx.textAlign="left";
    const code = selectedTab==="combat" ? ("C-"+String(st.id).padStart(2,"0")) : (tx("stageCodePrefix")+String(st.id).padStart(2,"0"));
    ...
    ctx.fillStyle="rgba(255,255,255,.68)";
    ctx.font="15px " + FONT_UI;
    ctx.fillText(stageDisplayDesc(st),W-330,265);
    ctx.fillText(tx("rewardLabel")+st.reward+" "+tx("crystalWord")+" / "+(st.exp||500)+" EXP",W-330,333);
    ctx.fillText(tx("typeLabel")+(selectedTab==="combat" ? st.type : missionLabel()),W-330,363);
    ctx.fillText(tx("statusLabel")+(done?tx("ownedStatus"):locked?ui("locked"):tx("availableStatus")),W-330,393);
    if(selectedTab==="combat") ctx.fillText("Lv."+st.lv,W-330,423);
...
function drawStory(){
  ...
  ctx.fillStyle="rgba(255,255,255,.70)"; ctx.font="14px " + FONT_UI; ctx.textAlign="left"; ctx.fillText(tx("stagePrefix")+String(selectedStage).padStart(2,"0")+" "+stageDisplayName(st),90,78);
...
function drawMaterialSettlement(){
  ...
  ctx.fillStyle="#7cc7ff";
  ctx.font="bold 40px " + FONT_UI;
  ctx.fillText(language==="en"?"Dungeon Complete":"副本完成",W/2,155);
  ...
  ctx.fillStyle="#7cffb2"; if(r.expBooks){ ctx.fillText((language==="en"?"Character Material +":"角色升级材料 +")+r.expBooks,W/2-190,y); y+=34; }
  ctx.fillStyle="#7cc7ff"; if(r.weaponOre){ ctx.fillText((language==="en"?"Weapon Material +":"武器强化材料 +")+r.weaponOre,W/2-190,y); y+=34; }
...
function drawProtagonistArtBox(x,y,w,h,large=false){
  drawPortrait(x,y,w,h,roles[4],false);
}
function drawExecutorArt(i,x,y,w,h,large=false){
  // V40.8.3: all executors use the same placeholder model.
  // Flora is the original white-head / blue-body model, no illustration asset.
...
function operatorStatAtk(i){
  ...
  return Math.floor(100 + lv*(isProtagonist(i)?7:8) + roleWeaponLevelDisplay(i)*5);
}
function operatorStatHp(i){
  const lv=roleDisplayLevel(i);
  return Math.floor((isProtagonist(i)?1450:1200) + lv*(isProtagonist(i)?78:65));
}
function operatorStatDef(i){
  const lv=roleDisplayLevel(i);
  return Math.floor((isProtagonist(i)?120:90) + lv*(isProtagonist(i)?7:5));
}
...
function drawOperatorRightGrowthPanel(i,x,y,w,h){
  ctx.fillStyle="rgba(17,25,40,.82)"; ctx.fillRect(x,y,w,h);
  ctx.strokeStyle="rgba(124,199,255,.22)"; ctx.strokeRect(x,y,w,h);
  drawBtn(language==="en"?"Stats":"属性","",x+18,y+20,68,36,operatorTab==="level","#ffe066");
  drawBtn(language==="en"?"Skill":"技能","",x+94,y+20,68,36,operatorTab==="skill","#ffe066");
  drawBtn(language==="en"?"Weapon":"武器","",x+170,y+20,68,36,operatorTab==="weapon","#ffe066");
  drawBtn(language==="en"?"File":"档案","",x+246,y+20,68,36,operatorTab==="break","#ffe066");
...
function drawOperatorDetailPage(){
  ...
  drawOperatorLeftStatsPanel(i,38,top,258,panelH);
...
function breakthroughPreviewBonus(i){
  ...
  if(st===0) return {hp:250,atk:35,def:20};
  if(st===1) return {hp:420,atk:55,def:32};
...
function drawOperatorStatsTab(i,x=780,y=112,w=302,h=506){
  ...
  if(auto){
    drawInsetLabel((language==="en"?"Game Lv. ":"游戏等级 Lv.")+playerLevel+" / 80",bx,by+84,258,32,"#fff","rgba(255,255,255,.07)","rgba(255,255,255,.13)",12,true,"center");
    ...
    else if(canBreakthrough(i)) btn=language==="en"?"Breakthrough":"突破";
    drawInsetLabel(canBreakthrough(i)?((language==="en"?"Cap ":"等级上限 ")+cap+" → "+BREAK_LEVEL_CAPS[roleBreakStage(i)+1]):(language==="en"?"EXP Book + Gold":"经验书 + 金币"),bx,by+232,258,34,"#bfe8ff","rgba(124,199,255,.08)","rgba(124,199,255,.18)",11,true,"center");
...
function skillMechanicText(i,key,lv){
  ...
  if(isProtagonist(i)){
    if(key==="normal") return language==="en" ? "Three-hit single-target slash." : "三段单体斩击。";
    if(key==="skill") return language==="en" ? "Core overload. High break." : "核心过载，高破盾。";
    return language==="en" ? "Gray Domain single-target burst." : "灰域斩击，单体爆发。";
  }
...
function performBreakthrough(i){
  ...
  if((weaponOre||0)<cost.mat || gold<cost.gold){
    showCenter(language==="en"?"Not enough materials":"晋升材料不足",70);
    return false;
  }
  weaponOre-=cost.mat;
  gold-=cost.gold;
  charData[i].breakStage=stage+1;
  showCenter(language==="en"?"Breakthrough Complete":"晋升完成",90);
...
function weaponCostGold(i){ const cd=charData[i]||{}; return 150+(cd.weaponLevel||1)*30; }
...
function upgradeWeaponSelected(i){
  ...
  const costGold=weaponCostGold(i);
  if((weaponOre||0)<=0 || gold<costGold){ showCenter(language==="en"?"Not enough weapon materials":"精炼材料不足",70); return; }
  if((cd.weaponLevel||1)>=60){ showCenter(language==="en"?"Weapon already MAX":"已达上限",70); return; }
  weaponOre--; gold-=costGold; cd.weaponLevel=(cd.weaponLevel||1)+1;
...
function drawOperatorWeaponTab(i,x=780,y=112,w=302,h=506){
  ...
  drawInsetLabel(language==="en"?"CURRENT WEAPON":"当前武器",bx,by,258,28,"#bfe8ff","rgba(124,199,255,.08)","rgba(124,199,255,.18)",12,true,"center");
  drawInsetLabel(weaponNameById(equipped)+"  Lv."+roleEquippedWeaponLevel(i),bx,by+38,258,34,"#fff","rgba(255,255,255,.08)","rgba(255,255,255,.14)",13,true,"center");
  drawInsetLabel((language==="en"?"Type: ":"类型：")+weaponTypeLabel(roleWeaponType(i)),bx,by+82,258,28,"#a8b6cc","rgba(255,255,255,.055)","rgba(255,255,255,.10)",11,true,"center");
  ...
  drawBtn(language==="en"?"Upgrade":"强化","CLICK",bx+134,y+h-78,106,46,isEq,"#ffe066");
}
...
function drawShop(){
  ...
  for(let i=0;i<tabs.length;i++){
    drawBtn(tabs[i][1],"",45+i*128,135,118,42,shopTab===tabs[i][0],shopTab===tabs[i][0]?"#ffe066":"#fff");
  }
  ...
  if(shopTab==="recommend"){
    ctx.fillStyle="rgba(255,224,102,.08)";
    ctx.fillRect(70,235,660,220);
    ctx.strokeStyle="rgba(255,224,102,.32)";
    ctx.strokeRect(70,235,660,220);
    drawFloraImageBox(530,238,180,214,.96);

    ctx.fillStyle="#ffe066";
    ctx.font="bold 30px " + FONT_UI;
    ctx.textAlign="left";
    ctx.fillText(mt("monthlyRecommended"),105,285);

    ctx.fillStyle="rgba(255,255,255,.78)";
    ctx.font="18px " + FONT_UI;
    ctx.fillText("Flora / 芙洛拉",105,325);
    ctx.font="15px " + FONT_UI;
    ctx.fillText(mt("floraLimitedLine"),105,355);
    ctx.fillText(tx("floraFeatureLine"),105,382);

    ctx.fillStyle="rgba(124,199,255,.16)";
    ctx.fillRect(760,235,250,100);
    ctx.strokeStyle="rgba(124,199,255,.28)";
    ctx.strokeRect(760,235,250,100);
    ctx.fillStyle="#7cc7ff";
    ctx.font="bold 22px " + FONT_UI;
    ctx.fillText(ui("monthly"),790,275);
    ctx.fillStyle="rgba(255,255,255,.70)";
    ctx.font="14px " + FONT_UI;
    ctx.fillText(mt("daily150Crystal"),790,305);

    ctx.fillStyle="rgba(255,255,255,.08)";
    ctx.fillRect(760,355,250,100);
    ctx.strokeStyle="rgba(255,255,255,.18)";
    ctx.strokeRect(760,355,250,100);
    ctx.fillStyle="#fff";
    ctx.font="bold 22px " + FONT_UI;
    ctx.fillText(ui("packs"),790,395);
    ctx.fillStyle="rgba(255,255,255,.70)";
    ctx.font="14px " + FONT_UI;
    ctx.fillText(mt("starterGrowthPacks"),790,425);

    ctx.fillStyle="rgba(255,255,255,.40)";
    ctx.font="13px " + FONT_UI;
    ctx.fillText(mt("placeholderBannerHint"),105,430);
  }
  ...
  if(shopTab==="weapon"){
    if(shopSubTab==="limited"){
      ctx.fillStyle="rgba(136,216,255,.10)"; ctx.fillRect(70,250,420,210);
      ctx.strokeStyle="rgba(136,216,255,.55)"; ctx.strokeRect(70,250,420,210);
      ctx.fillStyle="#88d8ff"; ctx.font="bold 30px " + FONT_UI; ctx.textAlign="left";
      ctx.fillText(mt("everwinterName"),105,302);
      ctx.fillStyle="rgba(255,255,255,.75)"; ctx.font="16px " + FONT_UI;
      ctx.fillText(mt("floraSignatureWeapon"),105,334);
      ctx.fillText(mt("everwinterDesc"),105,364);
      ctx.fillStyle=ownedWeapons.flora?"#7cc7ff":"#ffe066"; ctx.font="bold 22px " + FONT_UI;
      ctx.fillText(ownedWeapons.flora?ui("claimed"):"888 "+ui("crystal"),105,420);
    }else{
      ctx.fillStyle="rgba(255,255,255,.08)"; ctx.fillRect(70,250,680,160);
      ctx.strokeStyle="rgba(255,255,255,.14)"; ctx.strokeRect(70,250,680,160);
      ctx.fillStyle="#fff"; ctx.font="bold 24px " + FONT_UI; ctx.textAlign="left";
      ctx.fillText(mt("permanentWeaponDepot"),100,300);
      ctx.fillStyle="rgba(255,255,255,.68)"; ctx.font="16px " + FONT_UI;
      ctx.fillText(mt("permanentWeaponDesc"),100,340);
    }
  }
  ...
  if(shopTab==="crystal"){
    const packs=[["500","$0.99"],["1500","$4.99"],["3500","$10.99"],["5500","$15.99"],["7500","$19.99"]];
    for(let i=0;i<packs.length;i++){
      const x=70+i*185,y=245;
      ctx.fillStyle="rgba(124,199,255,.09)"; ctx.fillRect(x,y,165,130);
      ctx.strokeStyle="rgba(124,199,255,.25)"; ctx.strokeRect(x,y,165,130);
      ctx.fillStyle="#7cc7ff"; ctx.font="bold 28px " + FONT_UI; ctx.textAlign="center"; ctx.fillText(packs[i][0],x+82,y+55);
      ctx.fillStyle="rgba(255,255,255,.65)"; ctx.font="15px " + FONT_UI; ctx.fillText(packs[i][1],x+82,y+90);
    }
  }
  ...
  if(shopTab==="packs"){
    ctx.fillStyle="rgba(255,224,102,.08)"; ctx.fillRect(70,240,760,210); ctx.strokeStyle="rgba(255,224,102,.25)"; ctx.strokeRect(70,240,760,210);
    ctx.fillStyle="#ffe066"; ctx.font="bold 28px " + FONT_UI; ctx.textAlign="left"; ctx.fillText(mt("packsTitle"),100,295);
    ctx.fillStyle="rgba(255,255,255,.72)"; ctx.font="16px " + FONT_UI;
    ctx.fillText(mt("packsDesc"),100,335);
  }
...
function drawBattleUI(){
  ...
  ctx.fillRect(16,14,350,118);
  ctx.strokeStyle="rgba(255,255,255,.14)";
  ctx.strokeRect(16,14,350,118);
  ...
  ctx.fillStyle="rgba(255,255,255,.66)";
  ctx.font="13px " + FONT_UI;
  const meta = tx("stageCodePrefix")+String(selectedStage).padStart(2,"0")+"  |  "+tx("areaLabel")+area+"/3  |  "+tx("crystalWord")+": "+crystals;
  ...
  if((player.perfectDodgeTimer||0)>0 || (player.perfectBuff||0)>0){
...
function drawBattle(){ const sx=(Math.random()-.5)*shake,sy=(Math.random()-.5)*shake*.7; ctx.save();ctx.translate(sx,sy); const bg=ctx.createLinearGradient(0,0,0,H);bg.addColorStop(0,"#15172a");bg.addColorStop(1,"#090a10");ctx.fillStyle=bg;ctx.fillRect(-50,-50,W+100,H+100); drawBattleBackground();
  drawGrid(); drawProjectAreaObjects(); drawEffects(); const objs=enemies.filter(e=>e.alive).map(e=>({t:"e",y:e.y,e})); objs.push({t:"p",y:player.y}); objs.sort((a,b)=>a.y-b.y); for(const o of objs)o.t==="e"?drawEnemy(o.e):drawPlayer(); if(areaCleared&&area<3){ctx.fillStyle="#ffe066";ctx.fillRect(W-42,110,22,H-160);ctx.fillStyle="#fff";ctx.font="bold 22px " + FONT_UI;ctx.textAlign="right";ctx.fillText("NEXT AREA →",W-58,H/2);} if(flash>0){ctx.globalAlpha=flash/35;ctx.fillStyle="#fff";ctx.fillRect(-50,-50,W+100,H+100);ctx.globalAlpha=1;} ctx.restore(); drawUltimateOverlay(); drawKrosPhaseTransitionOverlay(); drawBossKrosTopBar(); drawBattleUI(); if(centerTimer>0){ctx.fillStyle="#fff";ctx.font="bold 46px " + FONT_UI;ctx.textAlign="center";ctx.fillText(centerText,W/2,H*.43);}
  if(actionPromptTimer>0){
    ctx.save();
    ctx.fillStyle="#ffe066";
    ctx.font="bold 28px " + FONT_UI;
...
function paLoadArea(areaId){
  const data=paAreaData(areaId);
  paState.area=areaId;
  paState.blocks=data.blocks;
  paState.objects=data.objects;
  paState.exit=data.exit;
  paState.player.x=data.spawn.x;
  paState.player.y=data.spawn.y;
  paState.player.vx=0;
  paState.player.vy=0;
  showCenter("Project Area / Area "+areaId,70);
  showActionPrompt(language==="en"?"Explore. Press F near objects.":"探索区域。靠近物件按F互动。",90);
}
function startProjectAreaTeam(){
  ...
  paState = {
    ...
    objects:[],
    exit:null,
    rewards:{gold:0,expBooks:0,weaponOre:0,expReward:0,chests:0,crates:0,terminals:0,files:0,resources:0,switches:0,areasCleared:0},
...
function paFinish(){
  ...
  if(paState.repeat){
    ...
    crystals += reward.crystal; totalCrystalsEarned += reward.crystal;
    addPlayerExp(reward.expReward);
...
function drawProjectArea(){
  ...
  const ex=paRect(paState.exit);
  ctx.fillStyle="rgba(255,224,102,.20)";
  ctx.strokeStyle="#ffe066"; ctx.lineWidth=3;
  ctx.fillRect(ex.x,ex.y,ex.w,ex.h); ctx.strokeRect(ex.x,ex.y,ex.w,ex.h);
  ctx.fillStyle="#ffe066"; ctx.font="bold 26px "+FONT_UI; ctx.textAlign="center"; ctx.fillText("→",paState.exit.x,paState.exit.y+8);
  ...
  ctx.fillText("Project Area  /  Area "+paState.area+" / "+paState.maxArea+(paState.repeat ? "  /  CLEARED" : ""),60,67);

  const rw=paState.rewards;
  ctx.fillStyle="rgba(255,255,255,.58)";
  ctx.font="13px "+FONT_UI;
  ctx.fillText("Chest "+rw.chests+"  Crate "+rw.crates+"  Terminal "+rw.terminals+"  File "+(rw.files||0)+"  Resource "+(rw.resources||0),520,67);
...
window.addActionRecordExp = addActionRecordExp;
window.addActionRecordTaskProgress = addActionRecordTaskProgress;
...
function drawProtagonistGrowthNotice(){
  if(protagonistGrowthNoticeTimer<=0) return;
  protagonistGrowthNoticeTimer -= frameScale;
  ctx.save();
  ctx.globalAlpha = clamp(protagonistGrowthNoticeTimer/30,0,1);
  ctx.fillStyle = "rgba(0,0,0,.56)";
  ctx.fillRect(W/2-190,92,380,46);
  ctx.strokeStyle = "rgba(124,199,255,.45)"; ctx.strokeRect(W/2-190,92,380,46);
  ctx.fillStyle = "#bfe8ff"; ctx.font = "bold 18px "+FONT_UI; ctx.textAlign="center";
  ctx.fillText(protagonistGrowthNotice, W/2, 122);
  ctx.restore();
}
...
function forceSaveOnExit(){
  ...
  try{
    if(!guestMode && cloudUser && hasCreatedProfile) autoCloudSaveNow(true);
  }catch(e){}
}
